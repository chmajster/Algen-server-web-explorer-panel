...
- Network Tools now converts non-allowlisted command failures to a stable public error and replaces raw `dig` diagnostic text returned by DNS lookups with a generic failure message; only exact, server-defined rate-limit/concurrency errors retain their public text.
...
- Added Network Tools coverage proving diagnostic text merely containing rate-limit wording does not bypass the exact public-message allowlist.
