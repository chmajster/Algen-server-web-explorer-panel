...
- Persisted LDAP Manager connection state now revalidates `servers_json`, TLS flags, timeouts and timestamps before exposing a connection. Malformed server entries are skipped and corrupt numeric values fall back safely instead of crashing list/get/connection workflows.
- Persisted LDAP Authentication state now normalizes server rows, enum-like settings, booleans, timeouts, cache TTL, mapping priorities and access-policy mode. Type-corrupted SQLite values can no longer crash the LDAP settings endpoint or the application bootstrap path that loads LDAP settings during startup.
...
- Proxmox inventory, storage, cluster, template and backup views now normalize numeric, boolean and collection fields returned by the Proxmox API. A malformed metric or unexpected response shape degrades only that field/view item instead of raising `int()`/`float()` conversion errors and failing the whole dashboard endpoint.
...
- Added LDAP Authentication and LDAP Manager transport regressions proving their `ldap3` candidate address lists contain the policy-checked numeric address, the original hostname remains available for TLS semantics, and automatic referrals are disabled.
- Added persisted LDAP Manager corruption coverage proving malformed server JSON, TLS flags, timeouts and timestamps degrade to validated defaults without crashing connection reads.
- Added persisted LDAP Authentication corruption coverage proving malformed settings, server rows and mapping priorities cannot break settings reads or application bootstrap.
- Added Proxmox inventory response-corruption coverage proving malformed node/storage metrics and string booleans degrade safely rather than failing inventory endpoints.
