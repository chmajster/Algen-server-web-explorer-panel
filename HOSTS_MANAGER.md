...
The registry owns hosts, groups/memberships, connection credentials, accepted SSH keys, sanitized facts, enrollment tokens, Git repository assignments, power profiles and host-correlated operations. Ansible Controller continues to own projects/playbooks, templates, schedules, executions, host results and logical host locks.
...
Token creation produces a cryptographically random, single-use token valid for at most 60 minutes; only its SHA-256 hash is stored. The raw value remains briefly in memory to generate the one-time `set -euo pipefail` script. The script sends bounded identity facts, supports `WEBNAS_ENROLL_ADDRESS`, requires HTTPS and neither installs packages nor modifies SSH. `/enroll` atomically consumes the bearer token, enforces its hostname pattern and creates an unapproved, unverified host.
...
The `/api/modules/hosts-manager` API covers dashboard/host CRUD, approval, fingerprints, test/facts, capability plan/execute, groups, inventory, enrollment, discovery, credentials, repositories, power, operations/SSE, diagnostics and checksummed backups.
...
2. Generate an `edge-*` script, enroll a matching host and verify token reuse fails.
