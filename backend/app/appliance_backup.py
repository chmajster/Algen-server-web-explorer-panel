...
_ARCHIVE_NAME = re.compile(
    r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}\.webnas-backup\.zip$"
)
...
def _atomic_copy(source: Path, target: Path, mode: int) -> None:
    ...
    temporary = target.with_name(
        f".{target.name}.webnas-restore-{os.getpid()}.tmp"
    )
    try:
...
class ApplianceBackupService:
    ...
    def _resources(
        ...
        *,
        ...
        if include_config and config_path.is_file() and not config_path.is_symlink():
            resources.append(
                Resource("config", "config.yaml", config_path, "file")
            )
        ...
        for source in sorted(self.data_root.rglob("*")):
            ...
            if source.suffix.casefold() in {".sqlite", ".sqlite3", ".db"}:
                resources.append(
                    Resource("data", relative_posix, source, "sqlite")
                )
                continue
            ...
            if size > MAX_AUXILIARY_FILE_BYTES:
                continue
            if (
                source.suffix.casefold() in SAFE_AUXILIARY_SUFFIXES
                or (
                    relative.parts
                    and relative.parts[0] in {"secrets", "settings"}
                )
            ):
                resources.append(
                    Resource("data", relative_posix, source, "file")
                )
        return resources
    ...
    def _archive_path(self, label: str) -> Path:
        ...
        candidate = self.backup_root / (
            f"webnas-{timestamp}-{label}{ARCHIVE_SUFFIX}"
        )
        counter = 1
        while candidate.exists():
            candidate = self.backup_root / (
                f"webnas-{timestamp}-{label}-{counter}{ARCHIVE_SUFFIX}"
            )
            counter += 1
        return candidate
    ...
    def list_backups(self) -> list[dict[str, Any]]:
        ...
        for path in sorted(
            self.backup_root.glob(f"*{ARCHIVE_SUFFIX}"),
            reverse=True,
        ):
            try:
                stat = path.stat()
    ...
    def create(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(
            prefix="webnas-appliance-backup-",
            ...
            manifest = {
                ...
                "config_included": any(
                    item["scope"] == "config" for item in records
                ),
                "resources": records,
            }
            ...
            temporary_archive = destination.with_suffix(
    ...
    def _resolve_archive(self, name_or_path: str | Path) -> Path:
        ...
        if candidate.is_absolute():
            ...
            if resolved.parent != self.backup_root.resolve(strict=False):
                raise BackupValidationError(
                    "backup archive is outside the appliance backup directory"
                )
            return resolved
        name = str(candidate)
    ...
    def _read_manifest(self, archive: zipfile.ZipFile) -> dict[str, Any]:
        ...
        if (
            ...
            or payload.get("format") != "webnas-appliance-backup"
        ...
        if _version_tuple(__version__) < _version_tuple(minimum):
            raise BackupValidationError(
                f"backup requires WebNAS >= {minimum}"
            )
        return payload
    ...
    def validate(self, name_or_path: str | Path) -> dict[str, Any]:
        ...
        with (
            ...
            if len(names) != len(set(names)):
                raise BackupValidationError(
                    "backup contains duplicate archive members"
                )
            infos = {info.filename: info for info in all_infos}
            ...
            for raw_record in manifest["resources"]:
                if not isinstance(raw_record, dict):
                    raise BackupValidationError(
                        "invalid backup resource record"
                    )
                member = _safe_relative(
                    str(raw_record.get("member", ""))
                )
                relative_path = _safe_relative(
                    str(raw_record.get("relative_path", ""))
                )
                scope = raw_record.get("scope")
                kind = raw_record.get("kind")
                if scope not in {"config", "data"} or kind not in {
                    "file",
                    "sqlite",
                }:
                    raise BackupValidationError(
                        "unsupported backup resource record"
                    )
                ...
                if info is None or info.is_dir():
                    raise BackupValidationError(
                        f"backup member is missing: {member}"
                    )
                recorded_size = raw_record.get("size")
                if (
                    info.file_size > MAX_MEMBER_BYTES
                    or not isinstance(recorded_size, int)
                    or recorded_size != info.file_size
                ):
                    raise BackupValidationError(
                        f"backup member size is invalid: {member}"
                    )
    ...
    def _target_for(self, scope: str, relative_path: str) -> Path:
        ...
        if scope == "config":
            if relative_path != "config.yaml":
                raise BackupValidationError(
                    "unsupported config restore target"
                )
            return _config_path()
        if scope != "data":
        ...
        if self.data_root not in target.parents:
            raise BackupValidationError(
                "restore target escapes the data directory"
            )
        if target.name == "sessions.sqlite3":
            raise BackupValidationError(
                "session state cannot be restored from an appliance archive"
            )
        return target
    ...
    def restore(
        ...
        *,
        ...
        with (
            zipfile.ZipFile(path, "r") as archive,
            tempfile.TemporaryDirectory(
                prefix="webnas-appliance-restore-",
                dir=self.backup_root,
            ) as temp_name,
        ):
            manifest = self._read_manifest(archive)
            ...
            for record in manifest["resources"]:
                ...
                if _sha256(staged) != str(record["sha256"]):
                    raise BackupValidationError(
                        f"backup checksum changed during restore: {member}"
                    )
                ...
                for index, (record, staged, target) in enumerate(staged_records):
                    ...
                    if target.exists():
                        if target.is_symlink():
                            raise BackupValidationError(
                                "restore target cannot be a symbolic link"
                            )
...
router = APIRouter(
    prefix="/api/system/appliance-backups",
    tags=["appliance-backup"],
)


@router.get("")
def backups(
    user: SessionUser = Depends(
        require_permission(Permission.MODULES_BACKUP_CREATE)
    ),
):
    del user
...
def create_backup(
    ...
    try:
        ...
        api_error(
            500,
            "APPLIANCE_BACKUP_FAILED",
            "Appliance backup failed",
...
def validate_backup(
    ...
    user: SessionUser = Depends(
        require_permission(Permission.MODULES_BACKUP_RESTORE)
    ),
):
    del user
    try:
        ...
        api_error(
            422,
            "APPLIANCE_BACKUP_INVALID",
            "Appliance backup validation failed",
            reason=type(error).__name__,
        )
...
def restore_backup(
    ...
    if not payload.dry_run and payload.confirmation_text != expected:
        api_error(
            ...
            f"Type {expected} to confirm appliance restore",
        )
    try:
        ...
        record_activity(
            ActivityCategory.administration,
            "appliance_backup_restore",
            user.username,
            target=payload.archive,
            status=ActivityStatus.failure,
            details={
        ...
        api_error(
            422 if isinstance(error, BackupValidationError) else 500,
            "APPLIANCE_RESTORE_FAILED",
