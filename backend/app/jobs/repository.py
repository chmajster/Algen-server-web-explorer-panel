...
class JobRepository:
    ...
    def create(self, *, job_type: str, module: str, created_by: str, metadata: dict[str, Any] | None = None,
        ...
        with self._lock, self._connect() as connection:
            connection.execute(
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                (job_id, job_type, module, name, description, status.value, priority.value, now, now, created_by,
                 json.dumps(metadata or {}, ensure_ascii=False, separators=(",", ":")), int(retryable), int(cancellable),
                 parent_job_id, retry_count, max_retries, timeout, correlation_id, dedup_key, total_steps),
            )
        job = self.get(job_id)
        if job is None:
    ...
    def list(self, *, status: JobStatus | None = None, module: str | None = None, job_type: str | None = None,
             created_by: str | None = None, since: float | None = None, until: float | None = None,
             limit: int = 100, offset: int = 0) -> JobPage:
        clauses: list[str] = []
        params: list[Any] = []
