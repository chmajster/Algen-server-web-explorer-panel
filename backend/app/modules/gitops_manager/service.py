...
class GitOpsService:
    ...
    def export_configuration(self) -> list[str]:
        ...
        (webnas / "modules.json").write_text(
            json.dumps({"generated_at": int(time.time()), "note": "Module credentials and Secrets Manager data are excluded."}, indent=2), encoding="utf-8"
        )
    ...
    def overview(self) -> dict[str, Any]:
        ...
        return {
            ...
            "local_changes": len(status.stdout.splitlines()),
            "ahead": ahead,
            "behind": behind,
        }
    ...
    def fetch(self, context: JobContext | None = None) -> dict[str, Any]:
        ...
        if context:
            context.set_progress(20, "Fetching remote", current_step="fetch")
        self._run([git, "fetch", "--prune", "origin"], timeout=120, check=True)
        if context:
            context.set_progress(100, "Remote fetched", current_step="verify")
        return self.overview()
    ...
    def pull(self, context: JobContext | None = None) -> dict[str, Any]:
        ...
        git = self._git_binary()
        settings = self.settings()
        branch = self._valid_ref(settings["branch"])
        if not settings["remote"]:
            raise GitOpsUnavailable("Git remote is not configured")
        self._ensure_clean_secret_scan()
        if context:
            context.set_progress(15, "Fetching remote", current_step="fetch")
        self._run([git, "fetch", "--prune", "origin"], timeout=120, check=True)
        if context:
            context.set_progress(60, "Fast-forwarding branch", current_step="merge")
        self._run([git, "merge", "--ff-only", f"origin/{branch}"], timeout=60, check=True)
        if context:
            context.set_progress(100, "Pull complete", current_step="verify")
        return self.overview()
    ...
    def push(self, context: JobContext | None = None) -> dict[str, Any]:
        ...
        self._ensure_clean_secret_scan()
        if context:
            context.set_progress(20, "Pushing branch", current_step="push")
        self._run([git, "push", "origin", f"HEAD:{branch}"], timeout=120, check=True)
        if context:
            context.set_progress(100, "Push complete", current_step="verify")
        return self.overview()

    def checkout_branch(self, branch: str) -> dict[str, Any]:
        branch = self._valid_ref(branch)
        self.init_repository()
        git = self._git_binary()
        ...
        self.configure(current["remote"], branch)
        return self.overview()
    ...
    def enqueue(self, action: str, actor: str):
        ...
        if action not in handlers:
            raise ValueError("unsupported GitOps job action")
        return jobs().submit_callable(
            job_type=f"gitops.{action}",
            module="gitops-config-manager",
            created_by=actor,
            handler=lambda context, _metadata: handlers[action](context),
            retryable=action in {"fetch", "push"},
            cancellable=False,
            priority=JobPriority.normal,
            max_retries=1,
            timeout=180,
            name=f"GitOps {action}",
            dedup_key=f"gitops.{action}",
        )
