...
def events(
    ...
    session_type: str = Query("", max_length=16),
    query: str = Query("", max_length=160),
    since: str = Query("7 days ago", max_length=64),
    user: SessionUser = Depends(current_user),
):
    authorize(user, "login_history.view")
    return _controlled(
        lambda: service().events(
            limit=limit, offset=offset, username=username, source_ip=source_ip, result=result, session_type=session_type, query=query, since=since
        )
    )
...
def terminate(payload: TerminateSessionInput, user: SessionUser = Depends(mutating_user)):
    ...
    result = _controlled(lambda: service().terminate_session(payload.session_id, actor=user.username))
