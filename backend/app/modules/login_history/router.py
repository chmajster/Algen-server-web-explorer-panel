...
@router.get("/overview")
def overview(user:SessionUser=Depends(current_user)):authorize(user,"login_history.view");return _controlled(service().overview)
@router.get("/events")
...
@router.get("/sessions")
def sessions(user:SessionUser=Depends(current_user)):authorize(user,"login_history.view");return _controlled(lambda:{"items":service().active_sessions()})
@router.get("/findings")
def findings(user:SessionUser=Depends(current_user)):authorize(user,"login_history.view");return _controlled(lambda:{"items":service().security_findings()})
...
def terminate(payload:TerminateSessionInput,user:SessionUser=Depends(mutating_user)):
    ...
    result=_controlled(lambda:service().terminate_session(payload.session_id,actor=user.username));record_activity(ActivityCategory.login,"login_session_terminate",user.username,target=payload.session_id,source="login-history");return result
