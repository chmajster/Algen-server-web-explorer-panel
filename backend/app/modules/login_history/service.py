...
class LoginHistoryService:
    ...
    def _journal(self, *, since: str = "24 hours ago", limit: int = 1000) -> list[dict[str, Any]]:
        ...
        selectors = [
            ...
            "_COMM=sudo", "+", "SYSLOG_IDENTIFIER=sudo", "+",
            "SYSLOG_IDENTIFIER=login", "+", "SYSLOG_IDENTIFIER=systemd-logind",
        ]
    ...
    def events(self, *, limit: int = 100, offset: int = 0, username: str = "", source_ip: str = "", result: str = "", session_type: str = "", query: str = "", since: str = "7 days ago") -> dict[str, Any]:
        ...
        total = len(events); safe_limit, safe_offset = min(max(limit, 1), 500), max(offset, 0)
    ...
    def security_findings(self, *, minutes: int = 5, brute_force_threshold: int = 20, spray_users: int = 5) -> list[dict[str, Any]]:
        ...
        by_ip: dict[str, list[dict[str, Any]]] = defaultdict(list); by_user: dict[str, list[dict[str, Any]]] = defaultdict(list)
        for event in events:
    ...
    def overview(self) -> dict[str, Any]:
        ...
        accounts = Counter(item["username"] for item in failed if item["username"]); findings = self.emit_findings()
