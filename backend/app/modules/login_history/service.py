...
class LoginHistoryService:
    ...
    def _timestamp(row: dict[str, Any]) -> float:
        ...
        return int(raw) / 1_000_000 if raw.isdigit() else time.time()
        ...
        if match:
            return {
                **base,
                ...
                "session_type": "local",
                "result": "success",
                "event": "login" if match.group("action").lower() == "opened" else "logout",
                "session_id": "",
                "terminal": "",
                "duration": None,
            }
        match = _SUDO_RE.search(message)
        if "sudo" in (str(row.get("SYSLOG_IDENTIFIER") or "") + str(row.get("_COMM") or "")).lower() and match:
            return {
                **base,
                "username": match.group("user"),
                "source_ip": "",
                "authentication_method": "sudo/pam",
                "session_type": "local",
                "result": "success",
                "event": "sudo",
                "session_id": "",
    ...
    def _last_events(self, *, failed: bool, limit: int) -> list[dict[str, Any]]:
        ...
        for line in result.stdout.splitlines():
            if not line.strip() or line.startswith(("wtmp begins", "btmp begins", "reboot", "shutdown")):
                continue
            parts = line.split()
            if len(parts) < 3:
                continue
            ...
            items.append(
                {
                    ...
                    "username": user,
                    "source_ip": source,
                    "authentication_method": "wtmp/btmp",
                    "session_type": "ssh" if source else "local",
    ...
    def events(
        ...
        *,
        ...
        if not events:
            events.extend(self._last_events(failed=False, limit=fetch_limit))
            events.extend(self._last_events(failed=True, limit=fetch_limit))
        if username:
        ...
        if query:
            ...
            events = [
                item
                for item in events
                if needle in " ".join(str(item.get(k, "")) for k in ("username", "source_ip", "authentication_method", "raw")).casefold()
            ]
    ...
    def active_sessions(self) -> list[dict[str, Any]]:
                ...
                sessions.append(
                    {
                        ...
                        "uid": uid,
                        "user": user,
                        "remote": values[1].strip().lower() == "yes" if len(values) > 1 else False,
                        "remote_ip": values[2].strip() if len(values) > 2 else "",
                        "terminal": values[3].strip() if len(values) > 3 else "",
                        "login_time": values[4].strip() if len(values) > 4 else "",
                        "state": values[5].strip() if len(values) > 5 else "unknown",
                    }
                )
            return sessions
        ...
        for index, line in enumerate(result.stdout.splitlines()):
            ...
            if len(parts) >= 4:
                sessions.append(
                    {
                        "session_id": f"who-{index}",
                        "user": parts[0],
                        "terminal": parts[1],
                        "login_time": " ".join(parts[2:4]),
                        "remote_ip": parts[-1].strip("()") if parts[-1].startswith("(") else "",
                        "state": "active",
                    }
                )
        return sessions
    ...
    def terminate_session(self, session_id: str, *, actor: str) -> dict[str, Any]:
        if broker_required():
            ...
            if response.exit_code != 0:
                raise RuntimeError((response.stderr or "session termination failed")[:500])
            ...
            try:
                alerts().fire(
                    AlertEvent(
                        ...
                        key=key,
                        title=f"Login security event: {finding['type'].replace('_', ' ')}",
                        object_ref=str(finding.get("source_ip") or finding.get("username") or ""),
                        details=finding,
                        severity=AlertSeverity.error if finding["severity"] == "error" else AlertSeverity.warning,
                    )
                )
            except Exception:
    ...
    def overview(self) -> dict[str, Any]:
        ...
        success = [i for i in recent if i["result"] == "success" and i["event"] == "login"]
        failed = [i for i in recent if i["result"] == "failure"]
        accounts = Counter(i["username"] for i in failed if i["username"])
        return {
            "successful_logins_24h": len(success),
            "failed_logins_24h": len(failed),
            "unique_source_ips": len({i["source_ip"] for i in recent if i["source_ip"]}),
            "active_sessions": len(self.active_sessions()),
            "most_attacked_account": accounts.most_common(1)[0][0] if accounts else "",
