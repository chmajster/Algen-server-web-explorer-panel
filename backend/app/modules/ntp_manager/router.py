...
from .service import NtpUnavailable, service
...
def _activity(actor: str, action: str, target: str = "", details: dict[str, Any] | None = None, *, failed: bool = False) -> None:
    record_activity(ActivityCategory.module, action, actor, target=target, details=details or {},
        status=ActivityStatus.failure if failed else ActivityStatus.success, source="ntp-manager")
...
def add_source(payload: NtpSourceInput, user: SessionUser = Depends(mutating_user)):
    ...
    current = [NtpSourceInput(server=item["server"], prefer=bool(item.get("prefer")), enabled=bool(item.get("enabled", True))) for item in service().sources() if item.get("server")]
    if payload.server not in {item.server for item in current}:
        current.append(payload)
    result = _controlled(lambda: service().save_sources(current))
...
def delete_source(server: str, confirm: bool = False, user: SessionUser = Depends(mutating_user)):
    ...
    if not confirm:
        api_error(422, "CONFIRMATION_REQUIRED", "NTP configuration changes require confirmation")
    current = [NtpSourceInput(server=item["server"], prefer=bool(item.get("prefer")), enabled=bool(item.get("enabled", True))) for item in service().sources() if item.get("server") and item["server"] != server]
    result = _controlled(lambda: service().save_sources(current))
...
def service_action(payload: ServiceActionInput, user: SessionUser = Depends(mutating_user)):
    ...
    result = _controlled(lambda: service().service_action(payload.action))
