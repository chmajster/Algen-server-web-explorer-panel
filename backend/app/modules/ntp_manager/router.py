...
register_infrastructure_permissions(); router = APIRouter(prefix="/api/modules/ntp-manager", tags=["ntp-manager"])

def _activity(actor: str, action: str, target: str = "", details: dict[str, Any] | None = None, *, failed: bool = False) -> None:
    record_activity(ActivityCategory.module, action, actor, target=target, details=details or {}, status=ActivityStatus.failure if failed else ActivityStatus.success, source="ntp-manager")

def _controlled(operation):
    try: return operation()
    except NtpUnavailable as error: api_error(503, "NTP_UNAVAILABLE", str(error))
    except PermissionError as error: api_error(503, "NTP_PERMISSION_DENIED", str(error))
    except (OSError, RuntimeError) as error: api_error(502, "NTP_OPERATION_FAILED", str(error))
    except ValueError as error: api_error(422, "NTP_VALIDATION_FAILED", str(error))

def _managed_sources() -> list[NtpSourceInput]:
    instance = service(); backend = instance.detect_backend()
    if backend == NtpBackend.none: return []
...
def dashboard(user: SessionUser = Depends(current_user)):
    authorize(user, "ntp.view"); return _controlled(service().status)
@router.get("/sources")
def sources(user: SessionUser = Depends(current_user)):
    authorize(user, "ntp.view"); return _controlled(lambda: {"items": service().sources()})
@router.post("/sources")
...
def resync(user: SessionUser = Depends(mutating_user)):
    authorize(user, "ntp.resync"); job = _controlled(lambda: service().enqueue_resync(user.username)); _activity(user.username, "ntp_resync", details={"job_id": job.id}); return job
@router.post("/service")
