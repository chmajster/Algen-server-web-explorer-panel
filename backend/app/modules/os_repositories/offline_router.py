...
def create_export(payload: OfflineExportInput, user: SessionUser = Depends(require_permission(Permission.OS_REPOSITORIES_SNAPSHOTS_MANAGE))):
    ready()
    return controlled(lambda: offline_service().create_bundle(payload, user.username))
...
def verify_bundle(staged_id: str, user: SessionUser = Depends(require_permission(Permission.OS_REPOSITORIES_VIEW))):
    ready()
    return controlled(lambda: offline_service().verify_staged(staged_id))
...
def import_bundle(staged_id: str, payload: OfflineImportInput, user: SessionUser = Depends(require_permission(Permission.OS_REPOSITORIES_PACKAGES_UPLOAD))):
    ...
    return controlled(lambda: offline_service().import_staged(staged_id, payload, user.username))
