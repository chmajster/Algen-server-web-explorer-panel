        ...
        for address in addresses:
            ...
            try:
                ...
                if remaining is not None and remaining > 0:
                    response.close()
                    raise http.client.IncompleteRead(content, remaining)
                try:
                    return loads_with_depth_limit(content.decode("utf-8")) if content else {}
                except (ValueError, RecursionError) as error:
                    raise RuntimeError("Module API returned invalid JSON") from error
            except RuntimeError:
                raise
            except (OSError, http.client.HTTPException) as error:
                # A server may have applied a command before its response was
                # lost. Replaying it against the next address can duplicate it.
                if method.upper() not in {"GET", "HEAD"}:
                    raise RuntimeError("Module API request failed; the operation outcome is unknown") from error
                last_error = error
            finally:
                connection.close()
