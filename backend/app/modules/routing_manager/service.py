...
class RoutingService:
    ...
    def backend(self) -> dict[str, Any]:
        ...
        def active(unit: str) -> bool:
            ...
            return self._run([systemctl, "is-active", "--quiet", unit], timeout=6).returncode == 0
    ...
    def rules(self) -> list[dict[str, Any]]:
        items: list[dict[str, Any]] = []; ip = self._ip()
        for family, flag in ((4, "-4"), (6, "-6")):
            ...
            for row in rows if isinstance(rows, list) else []:
                if isinstance(row, dict): items.append({"family": family, **row})
        return items
    ...
    def tables(self) -> list[dict[str, Any]]:
        ...
        if path.exists():
            for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
                ...
                if len(parts) == 2 and parts[0].isdigit(): names[parts[0]] = parts[1]
        route_counts: dict[str,int] = {}; rule_counts: dict[str,int] = {}
        ...
        return [{"id": int(table_id), "name": name, "routes": route_counts.get(name, route_counts.get(table_id,0)), "rules": rule_counts.get(name, rule_counts.get(table_id,0))} for table_id,name in sorted(names.items(), key=lambda item:int(item[0]))]
    ...
    def _route_args(self, action: str, payload: RouteInput) -> list[str]:
        family = self._family(payload); args = [self._ip(), "-4" if family == 4 else "-6", "route", action, payload.destination]
    ...
    def _current_route_raw(self, payload: RouteInput) -> str:
        family = self._family(payload); args = [self._ip(), "-4" if family == 4 else "-6", "route", "show", payload.destination]
        if payload.table != "main": args += ["table", payload.table]
    ...
    def preview(self, action: str, payload: RouteInput) -> dict[str, Any]:
        ...
        return {"action": action, "before": before, "after": " ".join(command[3:]), "command": command, "inverse": inverse,
                "requires_confirmation": True, "rollback_seconds": payload.rollback_seconds, "backend": self.backend(),
                "warnings": self._warnings(payload)}

    def _warnings(self, payload: RouteInput) -> list[str]:
        warnings: list[str] = []
        if payload.destination == "default": warnings.append("Changing the default route can disconnect WebNAS")
        if payload.persistent and not self.backend()["persistent_routes"]: warnings.append(f"Persistent routes are not supported by detected backend {self.backend()['name']}")
    ...
    def diagnostics(self, target: str) -> dict[str, Any]:
        ...
        except ValueError: raise ValueError("diagnostic target must be an IP address")
    ...
    def _read_transaction(self, transaction_id: str) -> dict[str, Any]:
        if not re_full_id(transaction_id): raise ValueError("invalid transaction id")
        path = self._transaction_path(transaction_id)
        if not path.exists(): raise LookupError("transaction not found")
        return json.loads(path.read_text(encoding="utf-8"))

    def _persist_networkmanager(self, action: str, payload: RouteInput) -> dict[str, Any]:
        if not payload.persistent: return {"persistent": False}
        backend = self.backend()
        if backend["name"] != "NetworkManager": raise RoutingUnavailable(f"Persistent routes are unsupported for {backend['name']}; runtime route was not made persistent")
        if not payload.interface: raise ValueError("persistent NetworkManager route requires an interface")
        nmcli = shutil.which("nmcli"); assert nmcli
        ...
        operator = f"+{family}.routes" if action == "replace" else f"-{family}.routes"
        result = self._run([nmcli, "connection", "modify", connection, operator, spec], timeout=20)
        if result.returncode != 0: raise RuntimeError((result.stderr or result.stdout or "nmcli route update failed")[:500])
        return {"persistent": True, "backend": "NetworkManager", "connection": connection, "route": spec}

    def apply_job(self, context: JobContext, metadata: dict[str, Any]) -> dict[str, Any]:
        action = str(metadata["action"]); payload = RouteInput.model_validate(metadata["payload"]); actor = str(metadata.get("actor") or "system")
        plan = self.preview(action, payload); transaction_id = uuid4().hex
        context.set_progress(15, "Snapshot captured", current_step="snapshot")
        result = self._run(plan["command"], timeout=20)
        if result.returncode != 0: raise RuntimeError((result.stderr or result.stdout or "route operation failed")[:500])
        persistent = self._persist_networkmanager(action, payload)
        ...
        if verify.returncode != 0:
            self._run(plan["inverse"], timeout=20)
            raise RuntimeError("route verification failed; previous route restored")
    ...
    def rollback(self, transaction_id: str, *, automatic: bool = False) -> dict[str, Any]:
        ...
        result = self._run(list(transaction["inverse"]), timeout=20)
        transaction["status"] = "rolled_back" if result.returncode==0 else "rollback_failed"
        transaction["rolled_back_at"] = time.time(); transaction["automatic"] = automatic
        transaction["rollback_error"] = "" if result.returncode==0 else (result.stderr or result.stdout)[:500]
    ...
    def policy_rule(self, action: str, payload: PolicyRuleInput) -> dict[str, Any]:
        if action not in {"add", "delete"}: raise ValueError("rule action must be add or delete")
        ip = self._ip(); args = [ip, "-4" if payload.family==4 else "-6", "rule", action]
