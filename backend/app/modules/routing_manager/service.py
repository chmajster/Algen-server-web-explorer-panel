...
class RoutingService:
    ...
    def tables(self) -> list[dict[str, Any]]:
        ...
        for route in self.routes():
            key = str(route.get("table", "main"))
            route_counts[key] = route_counts.get(key, 0) + 1
        for rule in self.rules():
            key = str(rule.get("table", rule.get("lookup", "main")))
            rule_counts[key] = rule_counts.get(key, 0) + 1
        return [
            {
                "id": int(table_id),
                "name": name,
                "routes": route_counts.get(name, route_counts.get(table_id, 0)),
                "rules": rule_counts.get(name, rule_counts.get(table_id, 0)),
            }
    ...
    def _family(payload: RouteInput) -> int:
        if payload.destination != "default":
            return ipaddress.ip_network(payload.destination).version
        if payload.gateway:
            return ipaddress.ip_address(payload.gateway).version
        if payload.source:
            return ipaddress.ip_address(payload.source).version
        return 4
    ...
    def _route_args(self, action: str, payload: RouteInput) -> list[str]:
        ...
        if payload.source:
            args += ["src", payload.source]
        if payload.metric is not None:
            args += ["metric", str(payload.metric)]
        if payload.table != "main":
    ...
    def _warnings(self, payload: RouteInput) -> list[str]:
        ...
        if payload.destination == "default":
            ...
            for route in existing:
                ...
                if route.get("family") != candidate.version or not destination or destination == "default":
                    continue
                try:
                    current = ipaddress.ip_network(str(destination), strict=False)
                except ValueError:
                    continue
                if current.overlaps(candidate) and str(current) != str(candidate):
                    overlaps.append(str(current))
            if overlaps:
                warnings.append(f"Overlapping routes detected: {', '.join(overlaps[:5])}")
        ...
        if payload.gateway and payload.interface:
            ...
            if lookup.returncode != 0:
                warnings.append("Gateway is not currently reachable through the selected interface")
        return warnings

    def preview(self, action: str, payload: RouteInput) -> dict[str, Any]:
        if action not in {"replace", "delete"}:
            raise ValueError("route action must be replace or delete")
        before = self._current_route_raw(payload)
        command = self._route_args(action, payload)
        if action == "replace":
            inverse = (
                [self._ip(), "-4" if self._family(payload) == 4 else "-6", "route", "replace", *shlex.split(before)]
                if before
                else self._route_args("delete", payload)
            )
        else:
            if not before:
    ...
    def diagnostics(self, target: str) -> dict[str, Any]:
        try:
            ...
            raise ValueError("diagnostic target must be an IP address") from error
        ...
        return {
            "target": address,
            "route_ok": lookup.returncode == 0,
            "route": lookup.stdout.strip()[:2000],
            "ping_available": bool(ping_binary),
            "ping_ok": bool(ping and ping.returncode == 0),
            "ping": (ping.stdout if ping else "")[:2000],
            "traceroute_available": bool(traceroute_binary),
            "traceroute": (trace.stdout if trace else "")[:5000],
        }
    ...
    def _persistent_networkmanager(self, action: str, payload: RouteInput, *, actor: str) -> dict[str, Any]:
        ...
        backend = self.backend()
        if backend["name"] != "NetworkManager":
            raise RoutingUnavailable(f"Persistent routes are unsupported for {backend['name']}")
        if not payload.interface:
            raise ValueError("persistent NetworkManager route requires an interface")
        ...
        if not connection or connection == "--":
            raise RoutingUnavailable("No active NetworkManager connection for interface")
        ...
        if snapshot.returncode != 0:
            raise RuntimeError((snapshot.stderr or "Unable to snapshot NetworkManager routes")[:500])
        spec = payload.destination
        if payload.gateway:
            spec += f" {payload.gateway}"
        if payload.metric is not None:
            spec += f" {payload.metric}"
        mutation = self._nmcli_mutate("add" if action == "replace" else "remove", connection, family, spec, actor=actor)
        if mutation.returncode != 0:
            raise RuntimeError((mutation.stderr or mutation.stdout or "nmcli route update failed")[:500])
        return {
            "persistent": True,
    ...
    def _write_transaction(self, data: dict[str, Any]) -> None:
        path = self._transaction_path(data["id"])
    ...
    def apply_job(self, context: JobContext, metadata: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = RouteInput.model_validate(metadata["payload"])
        actor = str(metadata.get("actor") or "webnas")
        plan = self.preview(action, payload)
        transaction_id = uuid4().hex
        context.set_progress(15, "Snapshot captured", current_step="snapshot")
        result = self._mutate_ip(plan["command"], actor=actor)
        if result.returncode != 0:
            raise RuntimeError((result.stderr or result.stdout or "route operation failed")[:500])
        ...
        transaction = {
            ...
            "verification": verify.stdout.strip()[:2000],
        }
    ...
    def enqueue(self, action: str, payload: RouteInput, actor: str):
        ...
        return jobs().submit_callable(
            job_type=f"routing.{action}",
            module="routing-manager",
            created_by=actor,
            handler=self.apply_job,
            metadata={"action": action, "payload": payload.model_dump(mode="json"), "actor": actor},
            retryable=False,
            cancellable=False,
            priority=JobPriority.critical if payload.destination == "default" else JobPriority.high,
            timeout=120,
            name=f"Routing {action}",
            total_steps=3,
        )

    def _schedule_rollback(self, transaction: dict[str, Any]) -> None:
        delay = max(0.0, float(transaction["expires_at"]) - time.time())
        timer = threading.Timer(delay, lambda: self.rollback(str(transaction["id"]), automatic=True))
        timer.daemon = True
        with self._lock:
    ...
    def confirm(self, transaction_id: str) -> dict[str, Any]:
        ...
        if transaction["status"] != "pending_confirmation":
            raise ValueError("transaction is not waiting for confirmation")
        transaction["status"] = "confirmed"
        transaction["confirmed_at"] = time.time()
        self._write_transaction(transaction)
        with self._lock:
            timer = self._timers.pop(transaction_id, None)
        if timer:
            timer.cancel()
        return transaction
    ...
    def rollback(self, transaction_id: str, *, automatic: bool = False) -> dict[str, Any]:
        transaction = self._read_transaction(transaction_id)
        if transaction["status"] not in {"pending_confirmation", "rollback_failed"}:
            return transaction
        actor = str(transaction.get("actor") or "webnas")
        errors: list[str] = []
        runtime_result = self._mutate_ip(list(transaction["inverse"]), actor=actor)
        if runtime_result.returncode != 0:
            errors.append((runtime_result.stderr or runtime_result.stdout or "runtime rollback failed")[:500])
        try:
            self._restore_persistent(dict(transaction.get("persistent") or {}), actor=actor)
        except Exception as error:
            errors.append(str(error)[:500])
        transaction["status"] = "rolled_back" if not errors else "rollback_failed"
        transaction["rolled_back_at"] = time.time()
        transaction["automatic"] = automatic
        transaction["rollback_error"] = "; ".join(errors)
        self._write_transaction(transaction)
        return transaction
    ...
    def policy_rule(self, action: str, payload: PolicyRuleInput, *, actor: str) -> dict[str, Any]:
        if action not in {"add", "delete"}:
            raise ValueError("rule action must be add or delete")
        args = [self._ip(), "-4" if payload.family == 4 else "-6", "rule", action]
        if payload.priority is not None:
            args += ["priority", str(payload.priority)]
        if payload.source != "all":
            args += ["from", payload.source]
        if payload.destination != "all":
            args += ["to", payload.destination]
        if payload.fwmark:
            args += ["fwmark", payload.fwmark]
        if payload.input_interface:
            args += ["iif", payload.input_interface]
        if payload.output_interface:
            args += ["oif", payload.output_interface]
        args += ["table", payload.table]
        result = self._mutate_ip(args, actor=actor)
        if result.returncode != 0:
            raise RuntimeError((result.stderr or result.stdout or "policy rule operation failed")[:500])
