...
def package_plan(module_id: str, action: PackageAction = PackageAction.install, remove_data: bool = False, user: SessionUser = Depends(mutating_user)):
    ...
    _authorize_module_package_action(module_id, action, user)
...
def _authorize_module_package_action(module_id: str, action: PackageAction, user: SessionUser) -> None:
