...
from .security import current_admin, mutating_admin, reauthenticate
...
def modules(
    ...
    user: SessionUser = Depends(current_admin),
):
...
def module_categories(user: SessionUser = Depends(current_admin)):
    return categories()


@router.get("/installed")
def installed_modules(user: SessionUser = Depends(current_admin)):
    return list_modules(installed_only=True)


@router.get("/updates")
def module_updates(user: SessionUser = Depends(current_admin)):
    return list_modules(updates_only=True)


@router.get("/jobs")
def package_jobs(status: str | None = None, module_id: str | None = None, limit: int = Query(200, ge=1, le=500), user: SessionUser = Depends(current_admin)):
    return repository().list_jobs(status=status, module_id=module_id, limit=limit)


@router.get("/jobs/{job_id}")
def package_job(job_id: str, user: SessionUser = Depends(current_admin)):
...
async def package_job_events(job_id: str, user: SessionUser = Depends(current_admin)):
    async def events():
        last = ""
        while True:
            job = repository().get_job(job_id)
...
def cancel_job(job_id: str, payload: AdminPackageAction, user: SessionUser = Depends(mutating_admin)):
...
def retry_job(job_id: str, payload: AdminPackageAction, user: SessionUser = Depends(mutating_admin)):
    reauthenticate(user, payload.admin_password)
    return manager(repository()).retry(job_id, user.username)


@router.get("/history")
def package_history(limit: int = Query(300, ge=1, le=1000), user: SessionUser = Depends(current_admin)):
    return repository().history(limit)


@router.get("/sources")
def package_sources(user: SessionUser = Depends(current_admin)):
    return repository().list_sources()


@router.post("/sources")
def create_source(payload: PackageSourceInput, user: SessionUser = Depends(mutating_admin)):
...
def update_source(source_id: str, payload: PackageSourceInput, user: SessionUser = Depends(mutating_admin)):
...
def delete_source(source_id: str, user: SessionUser = Depends(mutating_admin)):
...
def sync_source(source_id: str, user: SessionUser = Depends(mutating_admin)):
...
def module_logs(module_id: str, user: SessionUser = Depends(current_admin)):
...
def package_plan(module_id: str, action: PackageAction = PackageAction.install, remove_data: bool = False, user: SessionUser = Depends(current_admin)):
...
def install_module(module_id: str, payload: AdminPackageAction, user: SessionUser = Depends(mutating_admin)):
    return _enqueue_action(module_id, PackageAction.install, payload, user)


@router.post("/{module_id}/update")
def update_module(module_id: str, payload: AdminPackageAction, user: SessionUser = Depends(mutating_admin)):
    return _enqueue_action(module_id, PackageAction.update, payload, user)


@router.post("/{module_id}/uninstall")
def uninstall_module(module_id: str, payload: AdminPackageAction, user: SessionUser = Depends(mutating_admin)):
    return _enqueue_action(module_id, PackageAction.uninstall, payload, user)


@router.post("/{module_id}/start")
def start_module(module_id: str, payload: AdminPackageAction, user: SessionUser = Depends(mutating_admin)):
    return _enqueue_action(module_id, PackageAction.start, payload, user)


@router.post("/{module_id}/stop")
def stop_module(module_id: str, payload: AdminPackageAction, user: SessionUser = Depends(mutating_admin)):
    return _enqueue_action(module_id, PackageAction.stop, payload, user)


@router.post("/{module_id}/restart")
def restart_module(module_id: str, payload: AdminPackageAction, user: SessionUser = Depends(mutating_admin)):
