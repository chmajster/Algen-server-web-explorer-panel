...
from .settings_support import (
    ...
    ChownRequest,
    DesktopWidget as _DesktopWidget,
    GroupCreate,
    GroupMember,
    GroupPatch,
...
# Backwards-compatible public model export used by existing integrations/tests.
...
def system_resources(request: Request, user: SessionUser = Depends(_current_user)):
    authorize(user, "system.status")
    return request.app.state.resource_sampler.dashboard(user.username, is_admin=_is_admin(user.username))
...
def admin_auto_update_run(payload: UpdateAction, request: Request, user: SessionUser = Depends(_current_user)):
    ...
    return _run_auto_update_once(actor=user.username, force=True, update_config=payload.update_config, npm_audit_fix=payload.npm_audit_fix)
