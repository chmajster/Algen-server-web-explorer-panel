...
def test_lvm_inventory_normalizes_pvs_vgs_and_lvs() -> None:
    payloads = {
        "pvs": {"report": [{"pv": [{"pv_name": "/dev/sdb1", "vg_name": "data", "pv_size": "107374182400", "pv_free": "53687091200", "pv_attr": "a--"}]}]},
        "vgs": {"report": [{"vg": [{"vg_name": "data", "vg_size": "107374182400", "vg_free": "53687091200", "pv_count": "1", "lv_count": "1", "vg_attr": "wz--n-"}]}]},
        "lvs": {"report": [{"lv": [{"lv_name": "archive", "vg_name": "data", "lv_path": "/dev/data/archive", "lv_size": "53687091200", "lv_attr": "-wi-a-----", "pool_lv": "", "origin": "", "data_percent": "12.5", "metadata_percent": ""}]}]},
    }
    calls: list[list[str]] = []
    ...
    assert result["volume_groups"][0]["lv_count"] == 1
...
def test_fstab_inventory_marks_active_disabled_and_protected_mounts() -> None:
    content = """
# system mounts
UUID=root / ext4 defaults 0 1
/dev/sdb1 /srv/data xfs defaults,noatime 0 2
...
"""
    ...
    result = StorageDetailsService.parse_fstab(content, filesystems)
    ...
    assert data["current_source"] == "/dev/sdb1"
...
def test_diskstats_parser_filters_physical_devices_and_calculates_bytes() -> None:
    content = """
   8       0 sda 100 2 300 40 500 6 700 80 1 90 120
...
def test_details_service_rejects_unknown_tools_without_resolving_or_running() -> None:
    resolved: list[str] = []
