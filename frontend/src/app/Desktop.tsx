...
export function Desktop({ user, profile, language, theme, tasks, uploadControls, toasts, t, toast, onSettingsChange, onTheme, onLoggedOut }: {
  ...
  function renderApp(item: WindowInstance) {
    switch (item.app) {
      ...
      case "identity": return <Suspense fallback={<div className="loading-state">{t("status.loading")}</div>}><IdentityApp permissions={profile.permissions} t={t} toast={toast} onOpenPolicies={() => openApp("settings", "policies")} /></Suspense>;
      case "users": return <Suspense fallback={<div className="loading-state">{t("status.loading")}</div>}><IdentityApp permissions={profile.permissions} initialTab="users" t={t} toast={toast} onOpenPolicies={() => openApp("settings", "policies")} /></Suspense>;
      case "groups": return <Suspense fallback={<div className="loading-state">{t("status.loading")}</div>}><IdentityApp permissions={profile.permissions} initialTab="groups" t={t} toast={toast} onOpenPolicies={() => openApp("settings", "policies")} /></Suspense>;
