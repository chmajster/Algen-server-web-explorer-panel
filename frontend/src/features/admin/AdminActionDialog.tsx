...
export function AdminActionDialog({ title, fields, description, submitLabel, danger = false, t, onClose, onSubmit }: {
  title: string; fields: AdminField[]; description?: React.ReactNode; submitLabel?: string; danger?: boolean; t: Translate; onClose: () => void; onSubmit: (values: Record<string, string>) => Promise<void>;
}) {
  ...
  return <Modal title={title} closeLabel={t("action.close")} onClose={onClose} footer={<><button type="button" onClick={onClose}>{t("action.cancel")}</button><button className={danger ? "button-danger" : "button-primary"} disabled={saving} type="submit" form={formId}>{saving ? t("status.loading") : submitLabel || t("action.apply")}</button></>}>
