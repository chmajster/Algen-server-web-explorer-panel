...
const passwordField = (t: Translate): AdminField => ({ name: "admin_password", label: t("settings.adminPassword"), type: "password", required: true });
...
export function IdentityApp({ permissions, initialTab = "users", t, toast }: { permissions: string[]; initialTab?: Tab; t: Translate; toast: ToastFn }) {
  ...
  function createUser() {
    setDialog({ title: t("identity.user.create"), fields: [
      ...
      passwordField(t),
    ], submit: (values) => perform(() => api.createIdentityUser({ username: values.username, password: values.password, gecos: values.gecos, home: values.home || null, shell: values.shell || null, role: values.role, groups: [], allow: [], deny: [], create_home: true, admin_password: values.admin_password })) });
  }

  function createGroup() {
    setDialog({ title: t("identity.group.create"), fields: [{ name: "groupname", label: t("settings.groupName"), required: true }, passwordField(t)], submit: (values) => perform(() => api.createIdentityGroup({ groupname: values.groupname, allow: [], deny: [], admin_password: values.admin_password })) });
  }
  ...
  return <section className="identity-app">
    ...
    <nav className="identity-tabs" aria-label={t("identity.tabs")}>
      {([...(can("users.view") ? ["users" as const] : []), ...(can("groups.view") ? ["groups" as const] : []), ...(can("access.view") ? ["roles" as const, "history" as const] : [])] as Tab[]).map((name) => <button className={tab === name ? "active" : ""} key={name} onClick={() => { setTab(name); setSearch(""); }}>{name === "users" ? <Users /> : name === "groups" ? <UserCog /> : name === "roles" ? <ShieldCheck /> : <History />}<span>{t(`identity.tab.${name}`)}</span></button>)}
    </nav>
...
function UsersView({ users, selected, metadata, search, roleFilter, statusFilter, includeSystem, can, t, onSearch, onRole, onStatus, onSystem, onCreate, onSelect, onDialog, perform }: {
  ...
  onSearch: (value: string) => void; onRole: (value: string) => void; onStatus: (value: string) => void; onSystem: (value: boolean) => void; onCreate: () => void; onSelect: (user: IdentityUser | null) => void; onDialog: (value: Dialog) => void; perform: (action: () => Promise<unknown>) => Promise<void>;
}) {
  return <div className={`identity-workspace ${selected ? "has-details" : ""}`}><div className="identity-list-pane"><div className="identity-toolbar"><label className="identity-search"><Search /><input aria-label={t("action.search")} value={search} onChange={(event) => onSearch(event.target.value)} placeholder={t("identity.searchUsers")} /></label><select aria-label={t("rbac.role")} value={roleFilter} onChange={(event) => onRole(event.target.value)}><option value="">{t("identity.allRoles")}</option>{roleValues.map((role) => <option key={role} value={role}>{t(`rbac.role.${role}`)}</option>)}</select><select aria-label={t("identity.status")} value={statusFilter} onChange={(event) => onStatus(event.target.value)}><option value="">{t("identity.allStatuses")}</option><option value="active">{t("identity.active")}</option><option value="locked">{t("identity.locked")}</option></select><label className="identity-check"><input type="checkbox" checked={includeSystem} onChange={(event) => onSystem(event.target.checked)} />{t("identity.showSystem")}</label>{can("users.create") && <button className="button-primary" onClick={onCreate}><Plus />{t("identity.user.create")}</button>}</div><div className="identity-table-wrap"><table className="identity-table"><thead><tr><th>{t("settings.username")}</th><th>UID</th><th>{t("identity.primaryGroup")}</th><th>{t("rbac.role")}</th><th>{t("identity.status")}</th></tr></thead><tbody>{users.map((user) => <tr className={selected?.username === user.username ? "selected" : ""} key={user.username} tabIndex={0} onClick={() => onSelect(user)} onKeyDown={(event) => { if (event.key === "Enter") onSelect(user); }}><td><strong>{user.username}</strong><span className="identity-badges">{user.linux_admin && <small>{t("identity.linuxAdmin")}</small>}{user.is_system && <small>{t("identity.systemAccount")}</small>}</span></td><td>{user.uid}</td><td>{user.primary_group}</td><td>{t(`rbac.role.${user.role}`)}</td><td>{user.locked ? t("identity.locked") : t("identity.active")}</td></tr>)}</tbody></table></div></div>{selected && <UserDetails user={selected} metadata={metadata} can={can} t={t} onClose={() => onSelect(null)} onDialog={onDialog} perform={perform} />}</div>;
}

function UserDetails({ user, metadata, can, t, onClose, onDialog, perform }: { user: IdentityUser; metadata: PermissionMetadata[]; can: (permission: string) => boolean; t: Translate; onClose: () => void; onDialog: (value: Dialog) => void; perform: (action: () => Promise<unknown>) => Promise<void> }) {
  const [role, setRole] = useState<RbacRole>(user.role); const [policy, setPolicy] = useState<PolicyState>({ allow: user.allow, deny: user.deny });
  useEffect(() => { setRole(user.role); setPolicy({ allow: user.allow, deny: user.deny }); }, [user]);
  const editablePolicy = can("access.manage_user_permissions") && !user.linux_admin;
  function savePolicy() { onDialog({ title: `${t("identity.savePolicy")}: ${user.username}`, fields: [passwordField(t)], submit: (values) => perform(() => api.saveIdentityUserPolicy(user.username, { role, ...policy }, values.admin_password)) }); }
  function editLinuxAccount() { onDialog({ title: `${t("action.edit")}: ${user.username}`, fields: [...(can("users.rename") ? [{ name: "new_username", label: t("identity.newUsername"), value: user.username }] : []), { name: "gecos", label: t("identity.gecos"), value: user.gecos }, { name: "home", label: t("identity.home"), value: user.home }, { name: "shell", label: t("identity.shell"), value: user.shell }, passwordField(t)], submit: (values) => perform(() => api.updateIdentityUser(user.username, { new_username: values.new_username || user.username, gecos: values.gecos, home: values.home, shell: values.shell, groups_add: [], groups_remove: [], move_home: values.home !== user.home, admin_password: values.admin_password })) }); }
  function editQuota() { onDialog({ title: `${t("identity.quota")}: ${user.username}`, fields: [{ name: "soft_mb", label: t("identity.quotaSoft"), type: "number", value: "0", required: true }, { name: "hard_mb", label: t("identity.quotaHard"), type: "number", value: "0" }, { name: "mountpoint", label: t("identity.quotaMount"), value: "/" }, passwordField(t)], submit: (values) => perform(() => api.setIdentityUserQuota(user.username, Number(values.soft_mb), values.hard_mb ? Number(values.hard_mb) : null, values.mountpoint || null, values.admin_password)) }); }
  function simple(kind: "lock" | "unlock" | "password" | "delete") {
    const fields = kind === "password" ? [{ name: "new_password", label: t("settings.newPassword"), type: "password" as const, required: true }, { name: "force_change", label: t("identity.forcePasswordChange"), type: "select" as const, value: "false", options: [{ value: "false", label: t("common.no") }, { value: "true", label: t("common.yes") }] }, passwordField(t)] : kind === "delete" ? [{ name: "remove_home", label: t("identity.removeHome"), type: "select" as const, value: "false", options: [{ value: "false", label: t("common.no") }, { value: "true", label: t("common.yes") }] }, passwordField(t)] : [passwordField(t)];
    onDialog({ title: `${t(`identity.${kind}`)}: ${user.username}`, fields, danger: kind === "delete" || kind === "lock", submit: (values) => perform(() => kind === "lock" ? api.lockIdentityUser(user.username, values.admin_password, true) : kind === "unlock" ? api.lockIdentityUser(user.username, values.admin_password, false) : kind === "password" ? api.changeIdentityUserPassword(user.username, values.new_password, values.force_change === "true", values.admin_password) : api.deleteIdentityUser(user.username, values.admin_password, values.remove_home === "true")) });
  }
  return <aside className="identity-details"><header><div><strong>{user.username}</strong><span>{user.gecos || user.home}</span></div><button aria-label={t("action.close")} onClick={onClose}>×</button></header>{user.linux_admin && <div className="identity-protection"><ShieldCheck />{t("identity.linuxAdminProtection")}</div>}<dl><dt>UID / GID</dt><dd>{user.uid} / {user.gid}</dd><dt>{t("identity.home")}</dt><dd>{user.home}</dd><dt>{t("identity.shell")}</dt><dd>{user.shell}</dd><dt>{t("settings.groupsLabel")}</dt><dd>{user.groups.join(", ")}</dd></dl><div className="identity-actions">{can("users.update") && user.manageable && <button onClick={editLinuxAccount}><UserCog />{t("action.edit")}</button>}{can("users.manage_quota") && user.manageable && <button onClick={editQuota}>{t("identity.quota")}</button>}{can("users.lock") && user.manageable && !user.locked && <button onClick={() => simple("lock")}><Lock />{t("identity.lock")}</button>}{can("users.unlock") && user.manageable && user.locked && <button onClick={() => simple("unlock")}><Unlock />{t("identity.unlock")}</button>}{can("users.change_password") && user.manageable && <button onClick={() => simple("password")}><KeyRound />{t("identity.password")}</button>}{can("users.delete") && user.manageable && <button className="button-danger" onClick={() => simple("delete")}><Trash2 />{t("action.delete")}</button>}</div><label className="field-label">{t("rbac.role")}<select disabled={!editablePolicy || !can("access.manage_roles")} value={role} onChange={(event) => setRole(event.target.value as RbacRole)}>{roleValues.map((item) => <option key={item} value={item}>{t(`rbac.role.${item}`)}</option>)}</select></label><PermissionMatrix metadata={metadata} policy={policy} sources={user.permission_sources} disabled={!editablePolicy} t={t} onChange={setPolicy} />{editablePolicy && <button className="button-primary identity-save" onClick={savePolicy}>{t("identity.savePolicy")}</button>}</aside>;
}

function GroupsView({ groups, selected, metadata, search, includeSystem, can, t, onSearch, onSystem, onCreate, onSelect, onDialog, perform }: { groups: IdentityGroup[]; selected: IdentityGroup | null; metadata: PermissionMetadata[]; search: string; includeSystem: boolean; can: (permission: string) => boolean; t: Translate; onSearch: (value: string) => void; onSystem: (value: boolean) => void; onCreate: () => void; onSelect: (group: IdentityGroup | null) => void; onDialog: (value: Dialog) => void; perform: (action: () => Promise<unknown>) => Promise<void> }) {
  return <div className={`identity-workspace ${selected ? "has-details" : ""}`}><div className="identity-list-pane"><div className="identity-toolbar"><label className="identity-search"><Search /><input aria-label={t("action.search")} value={search} onChange={(event) => onSearch(event.target.value)} placeholder={t("identity.searchGroups")} /></label><label className="identity-check"><input type="checkbox" checked={includeSystem} onChange={(event) => onSystem(event.target.checked)} />{t("identity.showSystem")}</label>{can("groups.create") && <button className="button-primary" onClick={onCreate}><Plus />{t("identity.group.create")}</button>}</div><div className="identity-table-wrap"><table className="identity-table"><thead><tr><th>{t("settings.groupName")}</th><th>GID</th><th>{t("identity.members")}</th><th>{t("identity.inheriting")}</th></tr></thead><tbody>{groups.map((group) => <tr className={selected?.name === group.name ? "selected" : ""} key={group.name} onClick={() => onSelect(group)}><td><strong>{group.name}</strong>{group.protected && <small>{t("identity.protected")}</small>}</td><td>{group.gid}</td><td>{group.members.length}</td><td>{group.inheriting_count}</td></tr>)}</tbody></table></div></div>{selected && <GroupDetails group={selected} metadata={metadata} can={can} t={t} onClose={() => onSelect(null)} onDialog={onDialog} perform={perform} />}</div>;
}

function GroupDetails({ group, metadata, can, t, onClose, onDialog, perform }: { group: IdentityGroup; metadata: PermissionMetadata[]; can: (permission: string) => boolean; t: Translate; onClose: () => void; onDialog: (value: Dialog) => void; perform: (action: () => Promise<unknown>) => Promise<void> }) {
  const [policy, setPolicy] = useState<PolicyState>({ allow: group.allow, deny: group.deny }); useEffect(() => setPolicy({ allow: group.allow, deny: group.deny }), [group]);
  function credential(title: string, fields: AdminField[], danger: boolean, submit: (values: Record<string, string>) => Promise<unknown>) { onDialog({ title, fields: [...fields, passwordField(t)], danger, submit: (values) => perform(() => submit(values)) }); }
  return <aside className="identity-details"><header><div><strong>{group.name}</strong><span>GID {group.gid}</span></div><button aria-label={t("action.close")} onClick={onClose}>×</button></header>{group.protected && <div className="identity-protection"><ShieldCheck />{t("identity.groupProtection")}</div>}<div className="identity-actions">{can("groups.rename") && group.manageable && <button onClick={() => credential(t("identity.renameGroup"), [{ name: "new_name", label: t("identity.newGroupName"), value: group.name, required: true }], false, (values) => api.renameIdentityGroup(group.name, values.new_name, values.admin_password))}>{t("identity.renameGroup")}</button>}</div><h3>{t("identity.members")}</h3><div className="identity-member-list">{group.members.map((member) => <span key={member}>{member}{can("groups.manage_members") && group.manageable && !group.primary_users.includes(member) && <button aria-label={t("groups.removeMember")} onClick={() => credential(`${t("groups.removeMember")}: ${member}`, [], true, (values) => api.setIdentityGroupMember(group.name, member, false, values.admin_password))}>×</button>}</span>)}</div>{can("groups.manage_members") && group.manageable && <button onClick={() => credential(t("groups.addMember"), [{ name: "username", label: t("settings.username"), required: true }], false, (values) => api.setIdentityGroupMember(group.name, values.username, true, values.admin_password))}><Plus />{t("groups.addMember")}</button>}<h3>{t("identity.inheriting")}</h3><p className="identity-inheritors">{group.inheriting_users.join(", ") || t("common.none")}</p><h3>{t("identity.groupPolicy")}</h3><PermissionMatrix metadata={metadata} policy={policy} sources={{}} disabled={!can("access.manage_group_permissions") || !group.manageable} t={t} onChange={setPolicy} />{can("access.manage_group_permissions") && group.manageable && <button className="button-primary identity-save" onClick={() => credential(t("identity.savePolicy"), [], false, (values) => api.saveIdentityGroupPolicy(group.name, policy, values.admin_password))}>{t("identity.savePolicy")}</button>}{can("groups.delete") && group.manageable && <button className="button-danger identity-delete" onClick={() => credential(`${t("action.delete")}: ${group.name}`, [], true, (values) => api.deleteIdentityGroup(group.name, values.admin_password))}><Trash2 />{t("action.delete")}</button>}</aside>;
}
...
function PermissionMatrix({ metadata, policy, sources, disabled, t, onChange }: { metadata: PermissionMetadata[]; policy: PolicyState; sources: Record<string, string[]>; disabled: boolean; t: Translate; onChange: (value: PolicyState) => void }) {
  ...
  return <div className="permission-matrix"><label className="identity-search"><Search /><input aria-label={t("identity.searchPermissions")} value={query} onChange={(event) => setQuery(event.target.value)} /></label>{[...grouped.entries()].map(([category, items]) => <section key={category}><h4>{t(`permissions.category.${category}`)}</h4>{items.map((item) => { const state = policy.deny.includes(item.id) ? "deny" : policy.allow.includes(item.id) ? "allow" : "inherit"; return <label className={`permission-row risk-${item.risk}`} key={item.id}><span><strong>{t(item.label_key)}</strong><small>{item.id} · {t(`identity.risk.${item.risk}`)}{sources[item.id]?.length ? ` · ${sources[item.id].join(", ")}` : ""}</small></span><select disabled={disabled} value={state} onChange={(event) => set(item.id, event.target.value as "inherit" | "allow" | "deny")} aria-label={`${item.id} ${t("identity.permissionState")}`}><option value="inherit">{t("identity.inherited")}</option><option value="allow">{t("identity.allowed")}</option><option value="deny">{t("identity.denied")}</option></select></label>; })}</section>)}</div>;
}
