...
export function IdentityApp({ permissions, initialTab = "users", t, toast }: { permissions: string[]; initialTab?: Tab; t: Translate; toast: ToastFn }) {
  ...
  const accessibleTabs = useMemo<Tab[]>(() => [...(can("users.view") ? ["users" as const] : []), ...(can("groups.view") ? ["groups" as const] : []), ...(can("access.view") ? ["roles" as const, "history" as const] : [])], [can]);
