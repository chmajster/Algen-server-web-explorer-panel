...
export function IdentityApp({ permissions, initialTab = "users", t, toast }: { permissions: string[]; initialTab?: Tab; t: Translate; toast: ToastFn }) {
  ...
  function createUser() {
    setDialog({ title: t("identity.user.create"), fields: [
      ...
      passwordField(t),
    ], submit: (values) => perform(() => api.createIdentityUser({ username: values.username, password: values.password, gecos: values.gecos, uid: optionalNumber(values.uid), gid: optionalNumber(values.gid), home: values.home || null, shell: values.shell || null, role: values.role, groups: splitNames(values.groups), allow: [], deny: [], create_home: true, force_password_change: values.force_password_change === "true", admin_password: values.admin_password })) });
  }

  function createGroup() {
    setDialog({ title: t("identity.group.create"), fields: [{ name: "groupname", label: t("settings.groupName"), required: true }, { name: "gid", label: t("identity.optionalGid"), type: "number" }, passwordField(t)], submit: (values) => perform(() => api.createIdentityGroup({ groupname: values.groupname, gid: optionalNumber(values.gid), allow: [], deny: [], admin_password: values.admin_password })) });
  }
...
function UserDetails({ user, metadata, can, t, onClose, onDialog, perform }: { user: IdentityUser; metadata: PermissionMetadata[]; can: (permission: string) => boolean; t: Translate; onClose: () => void; onDialog: (value: Dialog) => void; perform: (action: () => Promise<unknown>) => Promise<void> }) {
  ...
  function editLinuxAccount() { onDialog({ title: `${t("action.edit")}: ${user.username}`, fields: [...(can("users.rename") ? [{ name: "new_username", label: t("identity.newUsername"), value: user.username }] : []), { name: "gecos", label: t("identity.gecos"), value: user.gecos }, { name: "home", label: t("identity.home"), value: user.home }, { name: "shell", label: t("identity.shell"), value: user.shell }, ...(can("users.manage_groups") ? [{ name: "groups_add", label: t("identity.groupsAdd") }, { name: "groups_remove", label: t("identity.groupsRemove") }] : []), { name: "force_password_change", label: t("identity.forcePasswordChange"), type: "select", value: String(user.password_change_required), options: [{ value: "false", label: t("common.no") }, { value: "true", label: t("common.yes") }] }, passwordField(t)], submit: (values) => perform(() => api.updateIdentityUser(user.username, { new_username: values.new_username && values.new_username !== user.username ? values.new_username : null, gecos: values.gecos, home: values.home !== user.home ? values.home : null, shell: values.shell, groups_add: splitNames(values.groups_add || ""), groups_remove: splitNames(values.groups_remove || ""), move_home: values.home !== user.home, force_password_change: values.force_password_change === "true", admin_password: values.admin_password })) }); }
