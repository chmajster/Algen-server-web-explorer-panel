...
export function IdentityApp({ permissions, initialTab = "users", embedded = false, t, toast, onOpenPolicies }: { permissions: string[]; initialTab?: Tab; embedded?: boolean; t: Translate; toast: ToastFn; onOpenPolicies?: () => void }) {
...
function UsersView({ users, selected, metadata, search, roleFilter, statusFilter, includeSystem, can, t, onSearch, onRole, onStatus, onSystem, onCreate, onSelect, onDialog, perform, onOpenPolicies }: {
...
function UserDetails({ user, metadata, can, t, onClose, onDialog, perform, onOpenPolicies }: { user: IdentityUser; metadata: PermissionMetadata[]; can: (permission: string) => boolean; t: Translate; onClose: () => void; onDialog: (value: Dialog) => void; perform: (action: () => Promise<unknown>) => Promise<void>; onOpenPolicies?: () => void }) {
  ...
  const editablePolicy = false;
  function savePolicy() { onDialog({ title: `${t("identity.savePolicy")}: ${user.username}`, fields: [], submit: () => perform(() => api.saveIdentityUserPolicy(user.username, { role, ...policy })) }); }
...
function GroupDetails({ group, metadata, can, t, onClose, onDialog, perform, onOpenPolicies }: { group: IdentityGroup; metadata: PermissionMetadata[]; can: (permission: string) => boolean; t: Translate; onClose: () => void; onDialog: (value: Dialog) => void; perform: (action: () => Promise<unknown>) => Promise<void>; onOpenPolicies?: () => void }) {
  ...
  return <aside className="identity-details"><header><div><strong>{group.name}</strong><span>GID {group.gid}</span></div><button aria-label={t("action.close")} onClick={onClose}>×</button></header>{group.protected && <div className="identity-protection"><ShieldCheck />{t("identity.groupProtection")}</div>}<div className="identity-actions">{can("groups.rename") && group.manageable && <button onClick={() => confirm(t("identity.renameGroup"), [{ name: "new_name", label: t("identity.newGroupName"), value: group.name, required: true }], false, (values) => api.renameIdentityGroup(group.name, values.new_name))}>{t("identity.renameGroup")}</button>}</div><h3>{t("identity.primaryUsers")}</h3><div className="identity-member-list">{group.primary_users.length ? group.primary_users.map((member) => <span key={member}>{member}</span>) : <span>{t("common.none")}</span>}</div><h3>{t("identity.supplementaryMembers")}</h3><div className="identity-member-list">{group.supplementary_members.length ? group.supplementary_members.map((member) => <span key={member}>{member}{can("groups.manage_members") && group.manageable && <button aria-label={t("groups.removeMember")} onClick={() => confirm(`${t("groups.removeMember")}: ${member}`, [], true, () => api.setIdentityGroupMember(group.name, member, false))}>×</button>}</span>) : <span>{t("common.none")}</span>}</div>{can("groups.manage_members") && group.manageable && <button onClick={() => confirm(t("groups.addMember"), [{ name: "username", label: t("settings.username"), required: true }], false, (values) => api.setIdentityGroupMember(group.name, values.username, true))}><Plus />{t("groups.addMember")}</button>}<h3>{t("identity.inheriting")}</h3><p className="identity-inheritors">{group.inheriting_users.join(", ") || t("common.none")}</p><h3>{t("identity.groupPolicy")}</h3><PermissionMatrix metadata={metadata} policy={policy} sources={{}} disabled t={t} onChange={setPolicy} />{onOpenPolicies && can("access.view") && <button className="button-primary identity-save" onClick={onOpenPolicies}>{t("identity.openPolicySettings")}</button>}{can("groups.delete") && group.manageable && <button className="button-danger identity-delete" onClick={() => confirm(`${t("action.delete")}: ${group.name}`, [], true, () => api.deleteIdentityGroup(group.name))}><Trash2 />{t("action.delete")}</button>}</aside>;
}

export function AccessPolicies({ permissions, t, toast }: { permissions: string[]; t: Translate; toast: ToastFn }) {
