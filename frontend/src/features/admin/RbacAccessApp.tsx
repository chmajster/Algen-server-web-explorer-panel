import { Check, Copy, Plus, RefreshCw, Search, Trash2 } from "lucide-react";
...
export function RbacAccessApp({ t: _t, toast }: { t: Translate; toast: ToastFn }) {
  ...
  const refresh = useCallback(async () => {
    ...
    } catch (error) { toast(error instanceof Error ? error.message : "Nie udało się pobrać RBAC", "error", "admin"); }
...
function RolesPanel({ roles, permissions, toast, refresh }: { roles: Role[]; permissions: PermissionItem[]; toast: ToastFn; refresh: () => Promise<void> }) {
  ...
  const toggle = (permission: string) => setGrants((current) => { const next = new Set(current); if (next.has(permission)) next.delete(permission); else next.add(permission); return next; });
  const setCategory = (category: string, enabled: boolean) => setGrants((current) => { const next = new Set(current); permissions.filter((p) => p.category === category).forEach((p) => { if (enabled) next.add(p.canonical); else next.delete(p.canonical); }); return next; });
  async function save() {
    ...
    try { if (selected) await request(`/api/rbac/roles/${encodeURIComponent(selected.id)}`, { method: "PUT", body: JSON.stringify(payload) }); else await request("/api/rbac/roles", { method: "POST", body: JSON.stringify(payload) }); toast("Rola zapisana", "ok", "admin"); setSelected(null); setName(""); setDescription(""); setGrants(new Set()); await refresh(); }
    catch (error) { toast(error instanceof Error ? error.message : "Błąd zapisu roli", "error", "admin"); }
  }
...
function LdapPanel({ groups, roles, toast, refresh }: { groups: ExternalGroup[]; roles: Role[]; toast: ToastFn; refresh: () => Promise<void> }) {
  ...
  async function test() { try { const result = await request<{ status: string }>("/api/ldap/test", { method: "POST", body: "{}" }); setStatus(result.status); } catch (e) { setStatus("Offline"); toast(e instanceof Error ? e.message : "LDAP offline", "error", "admin"); } }
  ...
  async function map() { if (!selectedGroup || !selectedRole) return; await request("/api/ldap/mappings", { method: "POST", body: JSON.stringify({ external_group_id: selectedGroup, role_id: selectedRole }) }); await refresh(); toast("Mapowanie LDAP zapisane", "ok", "admin"); }
...
function EffectivePanel({ permissions, toast }: { permissions: PermissionItem[]; toast: ToastFn }) {
  ...
  async function load() { try { setEffective(await request<Effective>(`/api/rbac/users/${encodeURIComponent(username)}/effective-permissions?auth_provider=${provider}&identity_id=${encodeURIComponent(identityId)}`)); } catch (e) { toast(e instanceof Error ? e.message : "Błąd", "error", "admin"); } }
