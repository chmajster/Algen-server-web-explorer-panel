...
export function StoreAppView({ t, toast }: { t: Translate; toast: ToastFn }) {
  ...
  const action = (app: StoreApp, name: "install" | "reinstall" | "uninstall" | "update" | "start" | "stop" | "restart") => setDialog({ title: `${t(`store.${name}`)}: ${app.manifest.name}`, fields: [], danger: ["uninstall", "stop", "restart"].includes(name), submit: async () => {
