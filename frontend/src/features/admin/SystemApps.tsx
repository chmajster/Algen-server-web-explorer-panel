...
export function UsersApp({ t, toast }: { t: Translate; toast: ToastFn }) {
  ...
  const simple = (user: AdminUser, action: "lock" | "unlock" | "delete") => setDialog({ title: `${t(`action.${action}`)}: ${user.username}`, fields: [passwordField(t)], danger: action === "delete", submit: (values) => perform(() => action === "lock" ? api.lockUser(user.username, values.admin_password) : action === "unlock" ? api.unlockUser(user.username, values.admin_password) : api.deleteUser(user.username, values.admin_password), state.refresh, toast, t) });
  const create = () => setDialog({ title: t("users.create"), fields: [{ name: "username", label: t("settings.username"), required: true }, { name: "password", label: t("settings.newPassword"), type: "password", required: true }, { name: "groups", label: t("settings.groupsLabel") }, passwordField(t)], submit: (values) => perform(() => api.createUser({ username: values.username, password: values.password, groups: values.groups.split(",").map((value) => value.trim()).filter(Boolean), create_home: true, admin_password: values.admin_password }), state.refresh, toast, t) });
  return <Shell title={t("app.users")} subtitle={t("users.subtitle")} loading={state.loading} t={t} onRefresh={state.refresh} actions={<button onClick={create}><Plus />{t("users.create")}</button>}><DataState state={state} t={t}>{state.data?.map((user) => <article className="data-row" key={user.username}><strong>{user.username}</strong><span>UID {user.uid}</span><span>{user.home}</span><span>{user.groups.join(", ")}</span><div className="data-actions"><button title={t("action.lock")} onClick={() => simple(user, "lock")}><Lock /></button><button title={t("action.unlock")} onClick={() => simple(user, "unlock")}><Unlock /></button><button title={t("users.password")} onClick={() => setDialog({ title: `${t("users.password")}: ${user.username}`, fields: [{ name: "new_password", label: t("settings.newPassword"), type: "password", required: true }, passwordField(t)], submit: (values) => perform(() => api.changeUserPassword(user.username, { new_password: values.new_password, admin_password: values.admin_password }), state.refresh, toast, t) })}><RotateCcw /></button><button className="danger" title={t("action.delete")} onClick={() => simple(user, "delete")}><Trash2 /></button></div></article>)}</DataState>{dialog && <AdminActionDialog {...dialog} t={t} onClose={() => setDialog(null)} onSubmit={dialog.submit} />}</Shell>;
}
...
export function GroupsApp({ t, toast }: { t: Translate; toast: ToastFn }) {
  ...
  const create = () => setDialog({ title: t("groups.create"), fields: [{ name: "groupname", label: t("settings.groupName"), required: true }, passwordField(t)], submit: (values) => perform(() => api.createGroup({ groupname: values.groupname, admin_password: values.admin_password }), state.refresh, toast, t) });
  return <Shell title={t("app.groups")} subtitle={t("groups.subtitle")} loading={state.loading} t={t} onRefresh={state.refresh} actions={<button onClick={create}><Plus />{t("groups.create")}</button>}><DataState state={state} t={t}>{state.data?.map((group) => <article className="data-row" key={group.name}><strong>{group.name}</strong><span>GID {group.gid}</span><span>{group.members.join(", ") || "—"}</span><div className="data-actions"><button title={t("groups.addMember")} onClick={() => setDialog({ title: `${t("groups.addMember")}: ${group.name}`, fields: [{ name: "username", label: t("settings.username"), required: true }, passwordField(t)], submit: (values) => perform(() => api.addGroupMember(group.name, { username: values.username, admin_password: values.admin_password }), state.refresh, toast, t) })}><Plus /></button><button title={t("groups.removeMember")} onClick={() => setDialog({ title: `${t("groups.removeMember")}: ${group.name}`, fields: [{ name: "username", label: t("settings.username"), required: true }, passwordField(t)], danger: true, submit: (values) => perform(() => api.removeGroupMember(group.name, values.username, values.admin_password), state.refresh, toast, t) })}><Unlock /></button><button className="danger" title={t("action.delete")} onClick={() => setDialog({ title: `${t("action.delete")}: ${group.name}`, fields: [passwordField(t)], danger: true, submit: (values) => perform(() => api.deleteGroup(group.name, values.admin_password), state.refresh, toast, t) })}><Trash2 /></button></div></article>)}</DataState>{dialog && <AdminActionDialog {...dialog} t={t} onClose={() => setDialog(null)} onSubmit={dialog.submit} />}</Shell>;
}
...
export function ServicesApp({ t, toast }: { t: Translate; toast: ToastFn }) {
  ...
  const action = (service: SystemdService, name: "start" | "stop" | "restart" | "enable" | "disable") => setDialog({ title: `${t(`services.${name}`)}: ${service.name}`, fields: [passwordField(t)], danger: name === "stop" || name === "restart", submit: (values) => perform(() => api.systemdServiceAction(service.name, name, values.admin_password, name === "restart"), state.refresh, toast, t) });
...
export function SambaAppView({ t, toast }: { t: Translate; toast: ToastFn }) {
  ...
  const service = (action: "start" | "stop" | "restart" | "reload") => setDialog({ title: t(`samba.${action}`), fields: [passwordField(t)], danger: action === "stop" || action === "restart", submit: (values) => perform(() => api.sambaService(action, values.admin_password), state.refresh, toast, t) });
  async function saveShares(config: SambaConfig, password: string) { await api.saveSambaConfig(config, password); toast(t("store.actionQueued"), "ok", "admin"); await state.refresh(); }
  async function removeShare(name: string, password: string) { const config = await api.appConfig("samba"); await saveShares({ ...config, shares: config.shares.filter((share) => share.name !== name) }, password); }
  function editShare(share?: SambaShare) { setDialog({ title: share ? t("samba.editShare") : t("samba.addShare"), fields: [{ name: "name", label: t("samba.shareName"), value: share?.name, required: true }, { name: "path", label: t("files.fullPath"), value: share?.path, required: true }, { name: "comment", label: t("samba.comment"), value: share?.comment }, { name: "valid_users", label: t("samba.validUsers"), value: share?.valid_users.join(", ") }, passwordField(t)], submit: async (values) => { const config = await api.appConfig("samba"); const nextShare: SambaShare = { name: values.name, path: values.path, comment: values.comment, enabled: true, browseable: share?.browseable ?? true, read_only: share?.read_only ?? false, guest_ok: share?.guest_ok ?? false, valid_users: values.valid_users.split(",").map((value) => value.trim()).filter(Boolean), create_mask: share?.create_mask || "0664", directory_mask: share?.directory_mask || "0775" }; await saveShares({ ...config, shares: [...config.shares.filter((item) => item.name !== (share?.name || values.name)), nextShare] }, values.admin_password); } }); }
  return <Shell title={t("app.samba")} subtitle={t("samba.subtitle")} loading={state.loading} t={t} onRefresh={state.refresh} actions={<><button onClick={() => editShare()}><Plus />{t("samba.addShare")}</button><button onClick={() => service("start")}><Play />{t("samba.start")}</button><button onClick={() => service("stop")}><Square />{t("samba.stop")}</button><button onClick={() => service("restart")}><RotateCcw />{t("samba.restart")}</button></>}><DataState state={state} t={t}><div className="summary-grid"><article><span>{t("samba.installed")}</span><strong>{state.data?.installed ? t("common.yes") : t("common.no")}</strong></article><article><span>{t("samba.shares")}</span><strong>{state.data?.shares.length || 0}</strong></article></div>{state.data?.shares.map((share) => <article className="data-row" key={share.name}><strong>{share.name}</strong><code>{share.path}</code><span className={`status-badge ${share.enabled ? "completed" : "cancelled"}`}>{share.enabled ? t("common.enabled") : t("common.disabled")}</span><div className="data-actions"><button onClick={() => editShare(share)}>{t("action.edit")}</button><button className="danger" onClick={() => setDialog({ title: `${t("action.delete")}: ${share.name}`, fields: [passwordField(t)], danger: true, submit: (values) => removeShare(share.name, values.admin_password) })}><Trash2 /></button></div></article>)}</DataState>{dialog && <AdminActionDialog {...dialog} t={t} onClose={() => setDialog(null)} onSubmit={dialog.submit} />}</Shell>;
}
