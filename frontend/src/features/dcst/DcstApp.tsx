...
export function DcstApp({ permissions, t, toast }: { permissions: string[]; t: Translate; toast: ToastFn }) {
  ...
  const tabCounts: Partial<Record<Tab, number>> = { services: services.length, tags: tags.length, ipsets: ipsets.length, ports: ports.length };
  const recent = (overview.recent_changes as Array<Record<string, unknown>>) || [];
  const activeServiceCount = services.filter((item) => item.state === "ACTIVE").length;
  const blockedServiceCount = services.filter((item) => item.blocked || item.state === "BLOCKED").length;
  const attentionServiceCount = services.filter((item) => item.state === "PENDING" || item.state === "ERROR").length;
  const activeRatio = services.length ? Math.round((activeServiceCount / services.length) * 100) : 0;
  const managedObjectCount = services.length + tags.length + ipsets.length + ports.length;
  const overviewMetrics = [
    { key: "services", label: "Services", value: overview.services ?? services.length, icon: Shield, tone: "primary" },
    { key: "active", label: "Active", value: overview.active_services ?? activeServiceCount, icon: CheckCircle2, tone: "healthy" },
    { key: "blocked", label: "Blocked", value: overview.blocked_services ?? blockedServiceCount, icon: Ban, tone: "danger" },
    { key: "ports", label: "Ports", value: overview.ports ?? ports.length, icon: Network, tone: "neutral" },
    { key: "ipsets", label: "IPSets", value: overview.ipsets ?? ipsets.length, icon: Boxes, tone: "neutral" },
    { key: "tags", label: "TAGS / APMID.ENV", value: overview.tags ?? tags.length, icon: Network, tone: "neutral" },
    { key: "rules", label: "Firewall rules", value: overview.firewall_rules ?? 0, icon: Wrench, tone: "neutral" },
  ];

  return <section className="system-app module-app dcst-app">
    <header className="feature-header dcst-header">
      <div className="dcst-header-identity"><span className="dcst-title-icon"><Shield /></span><div><span className="dcst-eyebrow">Network security control plane</span><h2>DATA Communication & Segmentation Tool <small>DCST</small></h2><p>Model, review and synchronize logical communication policies with Proxmox Firewall.</p></div></div>
      <div className="dcst-header-side"><div className="dcst-runtime-status"><span className={`dcst-status-dot ${loading ? "busy" : "ready"}`} /><div><strong>{loading ? "Refreshing policy inventory" : "Policy inventory loaded"}</strong><small>{managedObjectCount} managed object{managedObjectCount === 1 ? "" : "s"}</small></div></div><div className="header-actions"><button onClick={() => void refresh()} disabled={loading}><RefreshCw className={loading ? "spin" : ""} /> Refresh</button>{can("dcst.sync") && <button className="button-primary" onClick={() => setConfirm({ title: "Synchronize firewall", message: "Apply the complete DCST desired state to managed Proxmox Firewall objects? External rules will be preserved.", run: async () => { await dcstClient.firewallSync(false); await refresh(); success("Firewall synchronized"); } })}><Play /> Synchronize</button>}</div></div>
    </header>
    <nav className="module-tabs dcst-tabs" aria-label="DCST sections">{tabs.map(([id, label]) => <button key={id} className={tab === id ? "active" : ""} onClick={() => { setTab(id); if (id === "utilities") void loadUtilities(); }}><span>{label}</span>{tabCounts[id] !== undefined && <span className="dcst-tab-count">{tabCounts[id]}</span>}</button>)}</nav>

    {tab === "overview" && <div className="module-content dcst-overview">
      <div className="dcst-metric-grid">{overviewMetrics.map((metric) => { const Icon = metric.icon; return <article className={`data-card dcst-metric-card ${metric.tone}`} key={metric.key}><span className="dcst-metric-icon"><Icon /></span><div><small>{metric.label}</small><strong>{String(metric.value ?? 0)}</strong></div></article>; })}</div>
      <div className="dcst-overview-layout">
        <article className="data-card dcst-panel dcst-posture-card"><header><Shield /><div><strong>Policy posture</strong><small>Current desired-state service distribution</small></div></header><div className="dcst-posture-score"><strong>{activeRatio}%</strong><span>active</span></div><div className="dcst-progress" aria-label={`${activeRatio}% active services`}><span style={{ width: `${activeRatio}%` }} /></div><dl><div><dt>Active</dt><dd>{activeServiceCount}</dd></div><div><dt>Blocked</dt><dd>{blockedServiceCount}</dd></div><div><dt>Needs attention</dt><dd>{attentionServiceCount}</dd></div><div><dt>Managed VMs</dt><dd>{tags.reduce((sum, tag) => sum + tag.vm_count, 0)}</dd></div></dl></article>
        <article className="data-card dcst-panel dcst-recent-card"><header><Activity /><div><strong>Recent firewall changes</strong><small>Latest DCST policy and synchronization activity</small></div></header><div className="table-scroll"><table><thead><tr><th>Timestamp</th><th>User</th><th>Operation</th><th>Object</th><th>Status</th></tr></thead><tbody>{recent.map((row, index) => <tr key={String(row.id || index)}><td>{row.timestamp ? new Date(Number(row.timestamp) * 1000).toLocaleString() : "—"}</td><td>{String(row.user || "")}</td><td>{String(row.operation || "")}</td><td>{String(row.object_type || "")} / {String(row.object_id || "")}</td><td>{String(row.status || "")}</td></tr>)}</tbody></table></div>{!recent.length && <div className="dcst-inline-empty">No firewall changes recorded yet.</div>}</article>
      </div>
    </div>}

    {tab === "services" && <div className="module-content dcst-section"><div className="dcst-section-heading"><div><span className="dcst-eyebrow">Policy objects</span><h3>Communication services</h3><p>Control traffic between tags, IP sets and addresses with reusable policy objects.</p></div><span className="dcst-count-pill">{visibleServices.length} / {services.length}</span></div><div className="module-section-toolbar dcst-filter-bar"><div className="search-box"><Search /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search Services, Source, Destination..." /></div><select aria-label="Direction" value={direction} onChange={(event) => setDirection(event.target.value)}><option value="">All directions</option><option>IN</option><option>OUT</option></select><select aria-label="Action" value={action} onChange={(event) => setAction(event.target.value)}><option value="">All actions</option><option>ACCEPT</option><option>DROP</option><option>REJECT</option></select><select aria-label="State" value={state} onChange={(event) => setState(event.target.value)}><option value="">All states</option><option>ACTIVE</option><option>BLOCKED</option><option>DISABLED</option><option>PENDING</option><option>ERROR</option></select></div>
