...
export function DcstApp({ permissions, t, toast }: { permissions: string[]; t: Translate; toast: ToastFn }) {
  ...
  function notify(message: string) { toast(message, "success", "admin"); }
  async function saveService() {
    try {
      await dcstClient.saveService(serviceDraft, serviceEdit);
      notify(serviceEdit ? "Service updated" : "Service created");
      setServiceDraft(blankService); setServiceEdit(""); await refresh();
    } catch (error) { toast(error instanceof Error ? error.message : "DCST error", "error", "admin"); }
  }
  ...
  async function savePort() {
    try { await dcstClient.savePort(portDraft, portEdit); notify(portEdit ? "Port updated" : "Port created"); setPortDraft({ name: "", protocol: "tcp", port_from: 443, port_to: 443, description: "" }); setPortEdit(""); await refresh(); }
    catch (error) { toast(error instanceof Error ? error.message : "DCST error", "error", "admin"); }
  }
  async function saveIPSet() {
    try {
      ...
      notify(ipsetEdit ? "IPSet updated" : "IPSet created"); setIPSetDraft({ name: "", description: "", entries: "" }); setIPSetEdit(""); await refresh();
    } catch (error) { toast(error instanceof Error ? error.message : "DCST error", "error", "admin"); }
  }
  async function serviceAction(id: string, operation: "block" | "unblock" | "enable" | "disable") {
    try { await dcstClient.serviceAction(id, operation); notify(`Service ${operation} completed`); await refresh(); }
    catch (error) { toast(error instanceof Error ? error.message : "DCST error", "error", "admin"); }
  }
  async function bulk(operation: "block" | "unblock" | "enable" | "disable" | "sync") {
    if (!selected.size) return;
    try { await dcstClient.bulk(operation, [...selected]); notify(`Bulk ${operation} completed`); setSelected(new Set()); await refresh(); }
    catch (error) { toast(error instanceof Error ? error.message : "DCST error", "error", "admin"); }
  }
  ...
  return <section className="system-app module-app dcst-app">
    ...
    {confirm && <div className="dialog-backdrop" role="presentation"><div className="dialog-card" role="dialog" aria-modal="true" aria-labelledby="dcst-confirm-title"><h3 id="dcst-confirm-title">{confirm.title}</h3><p>{confirm.message}</p><div className="dialog-actions"><button onClick={() => setConfirm(null)}>Cancel</button><button className="button-primary" onClick={() => { const actionToRun = confirm.run; setConfirm(null); void actionToRun().catch((error) => toast(error instanceof Error ? error.message : "DCST error", "error", "admin")); }}><CheckCircle2 /> Confirm</button></div></div></div>}
