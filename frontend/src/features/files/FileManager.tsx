...
export function FileManager({ homePath, initialPath, settings, tasks, isAdmin, t, toast, onOpenFolderWindow, onShareSamba, onUpload, onUploadCancel, onUploadRetry, onSettingsChange }: {
  ...
  return <section ref={explorer} className="file-manager" aria-label={t("app.fileManager")}>
    ...
    {sambaRemoval && <AdminActionDialog title={t("files.removeSambaShare")} fields={[]} danger t={t} onClose={() => setSambaRemoval(null)} onSubmit={async () => { const share = sambaRemoval; const result = await api.removeSambaShare(share.name); toast(`${t("module.jobQueued")} ${result.job.id.slice(0, 8)}`, "ok", "admin"); setSambaRemoval(null); }} />}
