...
export function LogsApp({ permissions, t, toast }: { permissions: string[]; t: Translate; toast: ToastFn }) {
  ...
  const rowHeight = wrap ? (viewMode === "compact" ? 72 : 88) : viewMode === "compact" ? 42 : 58;
  ...
  return <section className={`logs-app ${sidebarCollapsed ? "sidebar-collapsed" : ""} ${selected ? "details-open" : ""}`}>
    <aside className="logs-sidebar">
      <header><strong><FolderTree />{t("logs.sources")}</strong><button type="button" aria-label={t("logs.collapseSources")} title={t("logs.collapseSources")} onClick={() => setSidebarCollapsed(true)}><PanelLeftClose /></button></header>
      <SourceTree groups={sources?.groups || []} services={services} containers={containers} selected={source} t={t} onChoose={chooseSource} />
      <section className="logs-saved"><header><strong><BookMarked />{t("logs.savedViews")}</strong><button type="button" title={t("logs.saveView")} aria-label={t("logs.saveView")} onClick={() => void saveView()}><Save /></button></header><div className="logs-saved-list">{savedViews.map((view) => <div key={view.id}><button className={source === view.source && query === view.query ? "active" : ""} title={view.name} onClick={() => applySaved(view)}><span>{view.name}</span></button>{!view.builtin && <button title={`${t("action.delete")}: ${view.name}`} aria-label={`${t("action.delete")}: ${view.name}`} onClick={() => void removeView(view)}><Trash2 /></button>}</div>)}</div></section>
    </aside>
    ...
    <main className="logs-main">
      <header className="logs-toolbar">
        ...
        <div className="logs-toolbar-group logs-toolbar-filter-group">
          <button type="button" className={filtersOpen ? "active" : ""} aria-expanded={filtersOpen} onClick={() => setFiltersOpen((value) => !value)}><Filter />{t("logs.filters")}<span>{activeFilters}</span></button>
          <button type="button" className="logs-icon-button" title={t("action.refresh")} aria-label={t("action.refresh")} onClick={() => void load(false)}><RefreshCw className={loading ? "spin" : ""} /></button>
        </div>
        {sources?.capabilities.live && <div className="logs-toolbar-group logs-toolbar-live-group"><button type="button" className={live ? "live active" : "live"} aria-pressed={live} onClick={() => setLive((value) => !value)}><span />{t("logs.live")}</button>{live && <button type="button" onClick={() => paused ? resume() : setPaused(true)}>{paused ? <CirclePlay /> : <CirclePause />}{paused ? t("logs.resume") : t("logs.pause")}{pending.length > 0 && <span>{pending.length}</span>}</button>}</div>}
        <div className="logs-toolbar-group logs-toolbar-export-group"><div className="logs-export"><Download aria-hidden="true" /><select aria-label={t("logs.export")} defaultValue="" onChange={(event) => { const value = event.target.value as "txt" | "json" | "jsonl" | "csv"; if (value) void exportView(value); event.target.value = ""; }}><option value="" disabled>{t("logs.export")}</option>{["txt", "json", "jsonl", "csv"].map((value) => <option key={value} value={value}>{value.toUpperCase()}</option>)}</select></div></div>
        <div className="logs-toolbar-group logs-toolbar-display-group">
          <button type="button" className={`logs-icon-button ${wrap ? "active" : ""}`} title={t("logs.wrap")} aria-label={t("logs.wrap")} aria-pressed={wrap} onClick={() => setWrap((value) => !value)}><WrapText /></button>
          <button type="button" className={`logs-icon-button ${autoScroll ? "active" : ""}`} title={t("logs.autoScroll")} aria-label={t("logs.autoScroll")} aria-pressed={autoScroll} onClick={() => setAutoScroll((value) => !value)}><Clock3 /></button>
...
function SourceTree({ groups, services, containers, selected, t, onChoose }: { groups: LogSourceGroup[]; services: LogService[]; containers: LogContainer[]; selected: string; t: Translate; onChoose: (value: string) => void }) {
  return <nav className="logs-source-tree" aria-label={t("logs.sources")}>{groups.map((group) => <details key={group.id} open><summary>{sourceIcons[group.id] || <FileText />}<span title={t(`logs.group.${group.id}`)}>{t(`logs.group.${group.id}`)}</span><ChevronRight /></summary><div>{group.items.map((item) => <button key={item.id} className={selected === item.id ? "active" : ""} disabled={!item.available} title={!item.available ? `${item.label} — ${t(`logs.status.${item.status}`)}` : item.label} onClick={() => onChoose(item.id)}><span className={`source-state ${item.status}`} /><span className="logs-source-copy"><span>{item.label}</span>{!item.available && <small>{t(`logs.status.${item.status}`)}</small>}</span></button>)}{group.id === "services" && services.map((service) => <button key={service.unit} className={selected === `service:${service.unit}` ? "active" : ""} title={`${service.unit} — ${service.description}`} onClick={() => onChoose(`service:${service.unit}`)}><span className={`source-state ${service.active}`} /><span className="logs-source-copy"><span>{service.unit}</span><small>{service.description}</small></span></button>)}{group.id === "containers" && containers.map((container) => <button key={container.id} className={selected === `container:${container.id}` ? "active" : ""} title={`${container.name} — ${container.image}`} onClick={() => onChoose(`container:${container.id}`)}><span className={`source-state ${container.state}`} /><span className="logs-source-copy"><span>{container.name}</span><small>{container.image}</small></span></button>)}</div></details>)}</nav>;
}
...
function LogRow({ entry, query, selected, t, style, onClick }: { entry: LogEntry; query: string; selected: boolean; t: Translate; style: React.CSSProperties; onClick: () => void }) {
  ...
  const origin = entry.unit || entry.identifier || entry.source;
  const severity = t(`logs.severity.${entry.severity}`);
  return <button style={style} role="listitem" aria-expanded={selected} className={`log-row severity-${entry.severity} ${entry.severity_reason === "python_traceback" ? "traceback" : ""} ${selected ? "selected" : ""}`} onClick={onClick}><time className="log-time" title={time}>{time}</time><span className="log-severity" title={severity}>{severity}</span><span className="log-source" title={origin}>{origin}</span><span className="log-pid" title={entry.pid === null ? undefined : `PID ${entry.pid}`}>{entry.pid ?? "—"}</span><span className="log-message" title={summary}><Highlight value={summary} query={query} /></span></button>;
}
