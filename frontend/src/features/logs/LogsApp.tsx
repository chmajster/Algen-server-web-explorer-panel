...
function LogRow({ entry, query, mode, selected, t, style, onClick }: { entry: LogEntry; query: string; mode: ViewMode; selected: boolean; t: Translate; style: React.CSSProperties; onClick: () => void }) {
  ...
  return <button style={style} role="listitem" className={`log-row severity-${entry.severity} ${selected ? "selected" : ""}`} onClick={onClick}>{mode === "table" ? <><time>{time}</time><span className="log-severity">{t(`logs.severity.${entry.severity}`)}</span><span title={entry.unit || entry.identifier || entry.source}>{entry.unit || entry.identifier || entry.source}</span><span>{entry.pid ?? "—"}</span><span className="log-message"><Highlight value={entry.message} query={query} /></span></> : <><time>{time}</time><span className="log-severity">{t(`logs.severity.${entry.severity}`)}</span><strong>{entry.unit || entry.identifier || entry.source}</strong>{entry.pid !== null && <small>PID {entry.pid}</small>}<span className="log-message"><Highlight value={entry.message} query={query} /></span></>}</button>;
}
