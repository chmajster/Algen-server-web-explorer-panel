import { Archive, Boxes, CircleAlert, CircleCheckBig, Database, Download, ExternalLink, FileText, Gauge, Link, PackageCheck, Play, RefreshCw, RotateCcw, Save, Shield, Square, Stethoscope, Trash2, Wrench } from "lucide-react";
...
export function ManagedModuleApp({ moduleId, permissions, t, toast }: { moduleId: string; permissions: string[]; t: Translate; toast: ToastFn }) {
  ...
  function openAction(action: string, payload: Record<string, unknown> = {}) { setDialog({ kind: "action", action, payload, fields: fieldsFor(action, t, payload, moduleId), danger: ["upgrade_all", "container_stop", "compose_down", "update_container", "app_remove", "remove_container"].includes(action) }); }
...
function fieldsFor(action: string, t: Translate, payload: Record<string, unknown> = {}, moduleId = ""): AdminField[] {
