...
export function ModuleHub({ t, toast, onOpen }: { t: Translate; toast: ToastFn; onOpen: (moduleId: string) => void }) {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const [catalogResult, runtimeResult] = await Promise.allSettled([api.apps(), api.modules()]);
      if (catalogResult.status === "rejected" && runtimeResult.status === "rejected") throw runtimeResult.reason;
