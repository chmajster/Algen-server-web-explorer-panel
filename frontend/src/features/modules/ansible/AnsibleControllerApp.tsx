...
type Props = { permissions: string[]; initialJobId?: string; initialScanId?: string; t: Translate; toast: ToastFn; onDeepLinkClose?: () => void };
...
export function AnsibleControllerApp({ permissions, initialJobId, initialScanId, t, toast, onDeepLinkClose }: Props) {
  ...
  useEffect(() => {
    if (initialJobId) setSection("jobs");
    else if (initialScanId) setSection("discovery");
  }, [initialJobId, initialScanId]);
  ...
  else if (section === "discovery") content = <Discovery scans={scans} initialScanId={initialScanId} canManage={can("ansible-controller.discovery")} t={t} toast={toast} refresh={refresh} onDeepLinkClose={onDeepLinkClose} />;
  ...
  else if (section === "jobs") content = <Jobs items={jobs} initialJobId={initialJobId} canCancel={can("ansible-controller.jobs.cancel")} canLaunch={can("ansible-controller.jobs.launch")} t={t} toast={toast} refresh={refresh} onDeepLinkClose={onDeepLinkClose} />;
  ...
  return <ModuleAppShell name={t("ansible.name")} status={status} section={section} sections={sections} t={t} onSection={(next) => { setSection(next); if ((initialJobId && next !== "jobs") || (initialScanId && next !== "discovery")) onDeepLinkClose?.(); }} actions={actions}>{loading && !dashboard ? <div className="loading-state">{t("status.loading")}</div> : content}</ModuleAppShell>;
}
...
function Hosts({ hosts, credentials, config, canManage, t, toast, refresh }: { hosts: AnsibleHost[]; credentials: AnsibleCredential[]; config: Record<string, unknown>; canManage: boolean; t: Translate; toast: ToastFn; refresh: () => Promise<void> }) {
  ...
  const [onboardingHostId, setOnboardingHostId] = useState<AnsibleHost["id"] | null>(null);
  ...
  async function accept() { if (!selected || !key || !(await confirmDialog(t(key.changed ? "ansible.host.confirmKeyChange" : "ansible.host.confirmKey"), t))) return; await api.acceptAnsibleHostKey(selected.id, key, key.changed); setKey(null); await refresh(); }
  async function onboard(host: AnsibleHost) {
  if (onboardingHostId === host.id) return;
  setOnboardingHostId(host.id);
  try {
    if (!(await confirmDialog(t("ansible.host.confirmOnboarding"), t))) return;
    const sudoProfile = config.managed_sudo_profile === "nopasswd" ? "nopasswd" : "none";
    const confirmHostName = sudoProfile === "nopasswd" ? (await promptDialog(t, t("ansible.host.typeAddressForSudo"), "")) || "" : "";
    if (sudoProfile === "nopasswd" && confirmHostName !== host.address) return;
    await api.onboardAnsibleHost({ host: { name: host.name, address: host.address, port: host.port, ssh_user: host.ssh_user, credential_id: host.credential_id, python_interpreter: host.python_interpreter, connection_type: host.connection_type, environment: host.environment, location: host.location, tags: host.tags, variables: host.variables, active: host.active }, initial_username: host.ssh_user, credential_id: host.credential_id, create_managed_user: true, managed_username: String(config.managed_username || "algen-ansible"), sudo_profile: sudoProfile, sudoers_policy: "", confirm_host_name: confirmHostName });
    toast(t("ansible.jobQueued"), "ok", "admin", "ansible-controller");
  } finally {
    setOnboardingHostId(null);
  }
}
  return <section className="ansible-panel"><header><div><h3>{t("ansible.hosts.title")}</h3><p>{t("ansible.hosts.hint")}</p></div><div className="header-actions"><label className="ansible-search"><Search /><input aria-label={t("action.search")} value={search} onChange={(event) => setSearch(event.target.value)} /></label>{canManage && <button onClick={() => setOpen((value) => !value)}><Plus />{t("ansible.host.add")}</button>}</div></header>{open && <form className="module-form-grid" onSubmit={save}><label>{t("common.name")}<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>{t("ansible.host.address")}<input required value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} /></label><label>{t("ansible.host.port")}<input type="number" min="1" max="65535" value={form.port} onChange={(event) => setForm({ ...form, port: event.target.value })} /></label><label>{t("ansible.host.sshUser")}<input value={form.ssh_user} onChange={(event) => setForm({ ...form, ssh_user: event.target.value })} /></label><label>{t("ansible.credential.title")}<select value={form.credential_id} onChange={(event) => setForm({ ...form, credential_id: event.target.value })}><option value="">{t("common.none")}</option>{credentials.filter((item) => ["ssh_private_key", "ssh_password"].includes(item.type)).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button className="button-primary" type="submit"><Save />{t("action.save")}</button></form>}{key && <div className="ansible-key-confirm" role="alert"><AlertTriangle /><div><strong>{t(key.changed ? "ansible.host.keyChanged" : "ansible.host.newKey")}</strong><code>{key.fingerprint}</code><p>{t("ansible.host.verifyFingerprint")}</p></div><button className="button-primary" onClick={() => void accept()}>{t("action.confirm")}</button><button onClick={() => setKey(null)}>{t("action.cancel")}</button></div>}<Table headers={[t("common.name"), t("ansible.host.address"), t("ansible.host.user"), t("ansible.host.fingerprint"), t("ansible.host.lastTest"), t("column.actions")]} rows={visible.map((host) => [<button className="link-button" onClick={() => setSelected(host)}>{host.name}</button>, `${host.address}:${host.port}`, host.ssh_user, <Status state={host.fingerprint_status} t={t} />, host.last_test_at ? new Date(host.last_test_at * 1000).toLocaleString() : t("common.none"), canManage && <div className="module-row-actions"><button onClick={() => void scanKey(host)}>{t("ansible.host.scanKey")}</button><button disabled={host.fingerprint_status !== "accepted"} onClick={() => void api.testAnsibleHost(host.id).then(() => toast(t("ansible.jobQueued"), "ok"))}>{t("ansible.host.test")}</button><button disabled={onboardingHostId === host.id || host.fingerprint_status !== "accepted" || !host.credential_id} onClick={() => void onboard(host)}>{t("ansible.host.onboard")}</button><button disabled={host.fingerprint_status !== "accepted"} onClick={() => void api.gatherAnsibleFacts(host.id).then(() => toast(t("ansible.jobQueued"), "ok"))}>{t("ansible.host.facts")}</button><button className="danger" onClick={async () => { if ((await confirmDialog(t("ansible.host.confirmDelete"), t))) void api.deleteAnsibleHost(host.id).then(refresh); }}>{t("action.delete")}</button></div>])} empty={t("ansible.hosts.empty")} />{selected && <HostDetails host={selected} t={t} onClose={() => setSelected(null)} />}</section>;
}
...
function AutomationAccount({ value, hosts, credentials, canManage, t, toast, refresh }: { value: Record<string, unknown>; hosts: AnsibleHost[]; credentials: AnsibleCredential[]; canManage: boolean; t: Translate; toast: ToastFn; refresh: () => Promise<void> }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    if (!(await confirmDialog(t("ansible.managedAccount.confirmSave"), t))) return;
    setSaving(true);
    try {
  ...
  async function rotate(host: AnsibleHost) { if (!(await confirmDialog(t("ansible.managedAccount.confirmRotate"), t))) return; setRotating(host.id); try { await api.rotateAnsibleHostKey(host.id); toast(t("ansible.managedAccount.rotationQueued"), "ok", "admin", "ansible-controller"); } catch (error) { toast(error instanceof Error ? error.message : t("error.generic"), "error"); } finally { setRotating(""); } }
...
function Credentials({ items, canManage, t, toast, refresh }: { items: AnsibleCredential[]; canManage: boolean; t: Translate; toast: ToastFn; refresh: () => Promise<void> }) {
  ...
  const [type, setType] = useState<AnsibleCredential["type"]>("ssh_password");
  ...
  function resetForm() {
    setName(""); setType("ssh_password"); setUsername(""); setSecret(""); setPassphrase(""); setDescription(""); setRevealSecret(false);
  }
  ...
  async function save(event: React.FormEvent) {
    event.preventDefault();
    if (!(await confirmDialog(t("ansible.credential.confirmSave"), t))) return;
    try {
  ...
  const types: AnsibleCredential["type"][] = ["ssh_password", "ssh_private_key", "become_password", "git_private_key", "awx_token", "vault_secret"];
    ...
    <div className="credential-workspace">
      <aside className="credential-vault">
        ...
        <div className="credential-list">{visible.map((item) => <article key={item.id}>
          ...
          {canManage && <button className="credential-delete" type="button" aria-label={`${t("action.delete")} ${item.name}`} onClick={async () => { if ((await confirmDialog(t("ansible.credential.confirmDelete"), t))) void api.deleteAnsibleCredential(item.id).then(refresh); }}><Trash2 /></button>}
...
function Playbooks({ items, projects, canManage, t, toast, refresh }: { items: AnsiblePlaybook[]; projects: AnsibleProject[]; canManage: boolean; t: Translate; toast: ToastFn; refresh: () => Promise<void> }) {
  ...
  async function remove(item: AnsiblePlaybook) { if (!(await confirmDialog(t("ansible.playbook.confirmDelete"), t))) return; try { await api.deleteAnsiblePlaybook(item.id); if (editing?.id === item.id) { setEditing(null); setEditorOpen(false); } toast(t("ansible.playbook.deleted"), "ok"); await refresh(); } catch (error) { toast(error instanceof Error ? error.message : t("error.generic"), "error"); } }
...
function Backups({ items, canManage, t, toast, refresh }: { items: ModuleBackup[]; canManage: boolean; t: Translate; toast: ToastFn; refresh: () => Promise<void> }) {
  const pendingRestores = useRef(new Set<string>());
  async function create() { try { await api.createAnsibleBackup(t("ansible.backup.description"), false); toast(t("ansible.jobQueued"), "ok"); await refresh(); } catch (error) { toast(error instanceof Error ? error.message : t("error.generic"), "error"); } }
  async function restore(item: ModuleBackup) {
    if (pendingRestores.current.has(item.id)) return;
    pendingRestores.current.add(item.id);
    try {
      if (!(await confirmDialog(t("ansible.backup.confirmRestore"), t))) return;
      await api.restoreAnsibleBackup(item.id, item.checksum);
      toast(t("ansible.jobQueued"), "ok");
    } catch (error) { toast(error instanceof Error ? error.message : t("error.generic"), "error"); }
    finally { pendingRestores.current.delete(item.id); }
  }
  async function remove(item: ModuleBackup) { if (!(await confirmDialog(t("ansible.backup.confirmDelete"), t))) return; await api.deleteAnsibleBackup(item.id); await refresh(); }
  return <section className="ansible-panel">{canManage && <div className="module-section-toolbar"><button className="button-primary" onClick={() => void create()}><Plus />{t("module.createBackup")}</button></div>}<ModuleBackups backups={items} t={t} onCreate={() => void create()} onRestore={(item) => void restore(item)} onDelete={(item) => void remove(item)} /></section>;
}
