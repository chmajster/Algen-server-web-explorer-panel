import { Activity, Archive, Bot, CalendarClock, Download, FileCog, FileText, Folder, GitBranch, Info, KeyRound, LayoutDashboard, Network, PlaySquare, Power, Radio, ScrollText, Server, Settings, Stethoscope, Tags, Users, Workflow } from "lucide-react";
import type { ModuleStatus } from "../../../api";
import type { Translate } from "../../../app/types";

export type ModuleSection = "overview" | "configuration" | "service" | "logs" | "diagnostics" | "backups" | "info" | "shares" | "users" | "sessions" | "hosts" | "inventory" | "discovery" | "credentials" | "automation-account" | "projects" | "playbooks" | "templates" | "jobs" | "schedules" | "facts" | "groups" | "enrollment" | "repositories" | "power" | "operations" | "settings" | "environment" | "installer" | "audit";

const icons: Record<ModuleSection, React.ReactNode> = { overview: <LayoutDashboard />, configuration: <FileCog />, service: <Server />, logs: <FileText />, diagnostics: <Stethoscope />, backups: <Archive />, info: <Info />, shares: <Folder />, users: <Users />, sessions: <Radio />, hosts: <Server />, inventory: <Workflow />, discovery: <Network />, credentials: <KeyRound />, "automation-account": <Bot />, projects: <GitBranch />, playbooks: <ScrollText />, templates: <PlaySquare />, jobs: <FileText />, schedules: <CalendarClock />, facts: <Info />, groups: <Users />, enrollment: <ScrollText />, repositories: <GitBranch />, power: <Power />, operations: <Activity />, settings: <Settings />, environment: <Tags />, installer: <Download />, audit: <Activity /> };
