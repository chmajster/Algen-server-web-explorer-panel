...
export function ModuleAppShell({ className, name, status, healthMessage, activeJob, section, sections, sectionLabels, t, actions, onSection, children }: { className?: string; name: string; status: ModuleStatus; healthMessage?: string; activeJob?: { operation: string; progress: number } | null; section: ModuleSection; sections: ModuleSection[]; sectionLabels?: Partial<Record<ModuleSection, string>>; t: Translate; actions?: React.ReactNode; onSection: (section: ModuleSection) => void; children: React.ReactNode }) {
  const rootClassName = ["module-app", className].filter(Boolean).join(" ");
  return <section className={rootClassName}><ModuleHeader name={name} status={status} healthMessage={healthMessage} activeJob={activeJob} t={t} actions={actions} /><div className="module-layout"><nav className="module-navigation" aria-label={t("module.sections")}>{sections.map((item) => <button key={item} data-section={item} type="button" className={section === item ? "active" : ""} aria-current={section === item ? "page" : undefined} onClick={() => onSection(item)}>{icons[item]}<span>{sectionLabels?.[item] || t(`module.section.${item}`)}</span></button>)}</nav><main className="module-content">{children}</main></div></section>;
}
