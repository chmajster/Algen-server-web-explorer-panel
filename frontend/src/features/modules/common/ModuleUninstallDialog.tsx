...
export function ModuleUninstallDialog({ item, activeShares = 0, activeSessions = 0, t, toast, onClose, onStarted }: { item: ModuleSummary; activeShares?: number; activeSessions?: number; t: Translate; toast: ToastFn; onClose: () => void; onStarted: (jobId: string) => void }) {
  ...
  const remembered = getRememberedAdminPassword();
  const [password, setPassword] = useState(remembered); const [remember, setRemember] = useState(Boolean(remembered)); const [mode, setMode] = useState<RemovalMode>("packages"); const [createBackup, setCreateBackup] = useState(true); const [confirmName, setConfirmName] = useState(""); const [plan, setPlan] = useState<PackagePlan | null>(null); const [loading, setLoading] = useState(true); const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await api.uninstallModule(item.id, password, { remove_config: mode !== "packages", remove_data: removeData, create_backup: createBackup, confirm_name: confirmName });
      if (remember) rememberAdminPassword(password); else forgetAdminPassword();
