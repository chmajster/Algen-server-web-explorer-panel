...
vi.mock("../../../api", async () => {
  ...
  return { ...actual, api: { ...actual.api, module: vi.fn(), osRepositoriesDashboard: vi.fn(), osRepositories: vi.fn(), osRepositoryPackages: vi.fn(), osRepositorySnapshots: vi.fn(), osRepositoryChannels: vi.fn(), osRepositoryJobs: vi.fn(), osRepositoryKeys: vi.fn(), osRepositoryAssignments: vi.fn(), osRepositoryHistory: vi.fn(), osRepositorySettings: vi.fn(), planOsRepository: vi.fn(), saveOsRepository: vi.fn() } };
});
...
describe("OsRepositoriesApp", () => {
  beforeEach(() => {
    ...
    vi.mocked(api.osRepositories).mockResolvedValue({ items: [{ id: "a".repeat(32), name: "Ubuntu", description: "Production packages", kind: "local", format: "apt", distribution: "ubuntu", distribution_version: "24.04", architectures: ["amd64"], source_url: "", active: true, schedule: "", retention_count: 10, last_sync_status: "completed", package_count: 2, size_bytes: 1024 }], page: 1, page_size: 50, total: 1 });
