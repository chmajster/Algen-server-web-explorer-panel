...
export function SambaModuleApp({ initialSharePath, readOnly = false, canReinstall = false, t, toast, onOpenFolder, onDirtyChange }: { initialSharePath?: string; readOnly?: boolean; canReinstall?: boolean; t: Translate; toast: ToastFn; onOpenFolder: (path: string) => void; onDirtyChange: (dirty: boolean) => void }) {
  ...
  useEffect(() => { if (!job || !["queued", "running"].includes(job.status) || typeof EventSource === "undefined") return; const source = new EventSource(apiUrl(`/api/modules/samba/jobs/${encodeURIComponent(job.id)}/events`), { withCredentials: true }); source.onmessage = (event) => { try { const next = JSON.parse(event.data) as ModuleJob; if (!next || typeof next.id !== "string" || typeof next.status !== "string") return; setJob(next); if (next.status === "completed") { if (next.action === "apply" || next.action === "restore") { setSavedConfig(config); } void refresh(true); if (next.result?.diagnostics) setDiagnostics(next.result.diagnostics as ModuleDiagnostic[]); } if (next.status === "failed") toast(next.error || t("module.jobFailed"), "error", "admin"); } catch { /* isolate malformed stream frames */ } }; source.onerror = () => source.close(); return () => source.close(); }, [config, job, refresh, t, toast]);
