...
export function NetworkMountsSettingsSection({ isAdmin, selectedJobId, t, toast, onSelectedJobClose }: { isAdmin: boolean; selectedJobId?: string; t: Translate; toast: ToastFn; onSelectedJobClose?: () => void }) {
  ...
  return <section className="network-mounts-settings" aria-label={t("settings.networkResources")} aria-busy={loading}>
    ...
    {error ? <div className="error-state" role="alert"><span>{error}</span><button type="button" onClick={() => void refresh()}>{t("action.retry")}</button></div>
      : !loading && mounts.length === 0 ? <NetworkMountEmptyState filtered={false} t={t} onCreate={() => setEditing("new")} onReset={resetFilters} />
        : !loading && visibleMounts.length === 0 ? <NetworkMountEmptyState filtered={filtered} t={t} onCreate={() => setEditing("new")} onReset={resetFilters} />
          : <div className="mount-card-grid">{visibleMounts.map((mount) => <NetworkMountCard key={mount.id} mount={mount} t={t} toast={toast} onEdit={() => setEditing(mount)} onAction={(action) => setActionDialog({ mount, action })} onLogs={() => showLogs(mount)} />)}</div>}
