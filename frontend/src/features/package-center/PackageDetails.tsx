...
export function PackageDetails({ item, t, onClose, onAction, onConfigure }: {
  ...
  const actions: PackageAction[] = item.state.installed
    ? [item.state.update_available ? "update" : "restart", "start", "stop", ...(item.manifest.removable ? ["uninstall" as const] : [])]
    : ["install"];
  ...
  return <Modal wide title={item.manifest.name} closeLabel={t("action.close")} onClose={onClose} footer={<>
    ...
    {item.manifest.configurable && onConfigure && <button type="button" onClick={onConfigure}>{t("package.configure")}</button>}
