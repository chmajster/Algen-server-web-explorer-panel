...
type PackageGridProps = {
  ...
  onToggleDesktopShortcut?: (item: ModuleSummary) => void;
  onAction: (item: ModuleSummary, action: PackageAction) => void;
  onShowJob: (item: ModuleSummary, job: AppJob) => void;
};

export function PackageGrid({ modules, loading, view, permissions, desktopShortcutModules, t, onDetails, onOpen, onToggleDesktopShortcut, onAction, onShowJob }: PackageGridProps) {
  ...
  return <div className={className}>
    {modules.map((item) => <PackageCard item={item} permissions={permissions} desktopShortcut={desktopShortcutModules?.has(item.id) ?? false} t={t} onDetails={() => onDetails(item)} onOpen={onOpen ? () => onOpen(item) : undefined} onToggleDesktopShortcut={onToggleDesktopShortcut ? () => onToggleDesktopShortcut(item) : undefined} onAction={(action) => onAction(item, action)} onShowJob={(job) => onShowJob(item, job)} key={item.id} />)}
  </div>;
}
