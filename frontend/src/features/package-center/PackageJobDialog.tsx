...
export function PackageJobDialog({ initialJob, jobId, moduleName, t, onClose }: {
  initialJob?: AppJob;
  jobId?: string;
  moduleName?: string;
  t: Translate;
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent("webnas:modules-changed", {
      detail: { moduleId: job.module_id, status: job.status, action: job.action },
    }));
  }, [job]);

  if (!job) {
    return (
      <Modal wide title={moduleName || t("package.liveJobTitle").replace("{name}", "")} closeLabel={t("action.close")} onClose={onClose}>
        {error
  ...
  const lines = logTail
    .map((entry) => `[${entry?.stream || "stdout"}] ${entry?.line || ""}`)
    .join("\n");

  return (
    <Modal
      wide
      title={t("package.liveJobTitle").replace("{name}", name)}
      closeLabel={t("action.close")}
      onClose={onClose}
      footer={(
        <>
          <span className="package-live-footer-note">{active ? t("package.backgroundJobHint") : t("package.operationFinished")}</span>
    ...
    >
      <section className={`package-live-job ${job.status}`}>
        <header>
          <div className="package-live-status" role="status" aria-live="polite">
            {statusIcon}
            <span><strong>{t(`task.${job.status}`)}</strong><small>{job.operation || job.action}</small></span>
          </div>
        ...
        <div className="package-live-log-header">
          ...
          {active && <span className={connected ? "connected" : "reconnecting"}>{t(connected ? "package.logConnected" : "package.logConnecting")}</span>}
        </div>
