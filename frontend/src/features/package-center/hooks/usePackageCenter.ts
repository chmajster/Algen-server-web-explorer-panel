...
export function usePackageCenter() {
  ...
  useEffect(() => {
    ...
    const events = activeIds.split("|").map((id) => {
      ...
      source.onmessage = (event) => {
        ...
        if (["completed", "failed", "cancelled"].includes(job.status)) void refresh(true);
      };
  ...
  return { modules, visibleModules, categories, jobs, history, sources, loading, error, tab, search, category, status, setTab, setSearch, setCategory, setStatus, refresh };
}
