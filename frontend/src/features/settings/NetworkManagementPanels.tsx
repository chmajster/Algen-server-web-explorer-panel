...
function Modal({ title, children, onClose }: { title: string; children: ReactNode; onClose: () => void }) {
  return <div className="network-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="network-modal" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <div className="network-form-grid">
        ...
        {(section === "ipv4" || section === "ipv6") && (() => {
          ...
          return <>
            ...
            {ip.method === "manual" && <><Field label="Adres"><input required placeholder={section === "ipv4" ? "192.0.2.10" : "2001:db8::10"} value={address?.address || ""} onChange={(e) => updateIp(section, { addresses: [{ address: e.target.value, prefix: address?.prefix ?? (section === "ipv4" ? 24 : 64) }] })} /></Field><Field label="Prefiks"><input required type="number" min={0} max={section === "ipv4" ? 32 : 128} value={address?.prefix ?? (section === "ipv4" ? 24 : 64)} onChange={(e) => updateIp(section, { addresses: [{ address: address?.address || "", prefix: Number(e.target.value) }] })} /></Field><Field label="Brama"><input value={ip.gateway || ""} onChange={(e) => updateIp(section, { gateway: e.target.value || null })} /></Field></>}
...
function GeneralPanel({ state, onChange }: { state: NetworkManagementState; onChange: (change: NetworkChange) => void }) {
...
function DnsForm({ state, onClose, onSubmit }: { state: NetworkManagementState; onClose: () => void; onSubmit: (dns: NonNullable<NetworkManagementState["managed"]["dns"]>) => void }) {
  ...
  return <Modal title="Konfiguracja DNS" onClose={onClose}><form onSubmit={(e) => { e.preventDefault(); onSubmit({ automatic: false, servers: servers.split(",").map((v) => v.trim()).filter(Boolean), search_domains: domains.split(",").map((v) => v.trim()).filter(Boolean), routing_domains: [], per_interface: {}, priority: 100, ignore_dhcp: true }); }}><div className="network-form-grid"><Field label="Serwery DNS"><input required value={servers} onChange={(e) => setServers(e.target.value)} placeholder="1.1.1.1, 2606:4700:4700::1111" /></Field><Field label="Domeny wyszukiwania"><input value={domains} onChange={(e) => setDomains(e.target.value)} /></Field></div><footer><button type="button" onClick={onClose}>Anuluj</button><button className="button-primary">Przejdź do planu</button></footer></form></Modal>;
}
...
function RouteForm({ initial, interfaces, onClose, onSubmit }: { initial?: ManagedNetworkRoute; interfaces: string[]; onClose: () => void; onSubmit: (route: ManagedNetworkRoute) => void }) {
  ...
  return <Modal title={initial ? "Edytuj trasę" : "Nowa trasa statyczna"} onClose={onClose}><form onSubmit={(e) => { e.preventDefault(); onSubmit(value); }}><div className="network-form-grid">
    ...
    <Field label="Metryka"><input type="number" min={0} value={value.metric} onChange={(e) => setValue({ ...value, metric: Number(e.target.value) })} /></Field><Field label="Tabela"><input type="number" min={1} value={value.table} onChange={(e) => setValue({ ...value, table: Number(e.target.value) })} /></Field>
...
export function RoutesPanel({ state, onChange }: { state: NetworkManagementState; onChange: (change: NetworkChange) => void }) {
  ...
  return <div className="network-management-stack"><div className="network-actionbar"><button className="button-primary" disabled={!state.provider.writable} onClick={() => setEditing(null)}><Plus />Dodaj trasę</button></div>
    <section className="network-management-card"><header><div><h3>Trasy zarządzane</h3><p>Trwałe trasy zapisane przez panel.</p></div></header><div className="monitor-table-wrap"><table><thead><tr><th>Nazwa</th><th>Rodzina</th><th>Cel</th><th>Brama / typ</th><th>Interfejs</th><th>Metryka</th><th>Akcje</th></tr></thead><tbody>{managed.length ? managed.map((route) => <tr key={route.id || route.name}><td>{route.name}</td><td>{route.family.toUpperCase()}</td><td><code>{route.destination}</code></td><td>{route.gateway || route.route_type}</td><td>{route.interface || "—"}</td><td>{route.metric}</td><td><button onClick={() => setEditing(route)}><Edit3 /></button><button onClick={() => onChange({ operation: "delete_route", object_id: route.id! })}><Trash2 /></button></td></tr>) : <tr><td colSpan={7}>Brak zarządzanych tras.</td></tr>}</tbody></table></div></section>
...
function TrafficForm({ initial, interfaces, onClose, onSubmit }: { initial?: NetworkTrafficRule; interfaces: string[]; onClose: () => void; onSubmit: (rule: NetworkTrafficRule) => void }) {
  ...
  return <Modal title={initial ? "Edytuj regułę ruchu" : "Nowa reguła ruchu"} onClose={onClose}><form onSubmit={(e) => { e.preventDefault(); onSubmit(value); }}><div className="network-form-grid">
    ...
    <Field label="Priorytet"><input type="number" min={1} max={10} value={value.priority} onChange={(e) => setValue({ ...value, priority: Number(e.target.value) })} /></Field>
  </div><footer><button type="button" onClick={onClose}>Anuluj</button><button className="button-primary">Przejdź do planu</button></footer></form></Modal>;
}
...
export function TrafficPanel({ state, onChange }: { state: NetworkManagementState; onChange: (change: NetworkChange) => void }) {
  ...
  return <div className="network-management-stack"><div className="network-actionbar"><button className="button-primary" disabled={!state.provider.writable || !state.tools.tc} onClick={() => setEditing(null)}><Plus />Dodaj regułę</button>{!state.tools.tc && <span>Brak narzędzia tc.</span>}</div>
    <section className="network-management-card"><div className="monitor-table-wrap"><table><thead><tr><th>Nazwa</th><th>Interfejs</th><th>Kierunek</th><th>Minimum</th><th>Maksimum</th><th>Priorytet</th><th>Akcje</th></tr></thead><tbody>{rules.length ? rules.map((rule) => <tr key={rule.id || rule.name}><td>{rule.name}</td><td>{rule.interface}</td><td>{rule.direction}</td><td>{rule.guaranteed_kbit} kbit/s</td><td>{rule.maximum_kbit} kbit/s</td><td>{rule.priority}</td><td><button onClick={() => setEditing(rule)}><Edit3 /></button><button onClick={() => onChange({ operation: "delete_traffic", object_id: rule.id! })}><Trash2 /></button></td></tr>) : <tr><td colSpan={7}>Brak reguł kontroli ruchu.</td></tr>}</tbody></table></div></section>
...
export function NetworkManagementWorkspace({ tab, t }: { tab: "general" | "interfaces" | "traffic" | "routes"; t: Translate }) {
  ...
  return <div className="network-management-workspace">
    ...
    {tab === "general" && <GeneralPanel state={state} onChange={(change) => void prepare(change)} />}
