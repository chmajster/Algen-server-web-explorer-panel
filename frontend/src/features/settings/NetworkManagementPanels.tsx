  ...
  return <Modal title={initial ? networkText("Edytuj interfejs") : networkText("Nowy interfejs")} onClose={onClose}>
    <form onSubmit={submit}>
      ...
      <div className="network-form-grid">
        {section === "link" && <>
          <Field label={networkText("Rodzaj")}><select value={value.kind} onChange={(e) => setValue({ ...value, kind: e.target.value as NetworkInterfaceConfiguration["kind"] })}><option value="physical">{networkText("Fizyczny")}</option><option value="bond">Bond</option><option value="vlan">VLAN</option><option value="bridge">Most</option></select></Field>
...
function GeneralPanel({ state, onChange, onNavigate }: { state: NetworkManagementState; onChange: (change: NetworkChange) => void; onNavigate: (tab: "interfaces" | "routes" | "connectivity") => void }) {
  ...
  return <div className="network-management-stack">
    <section className="network-management-cards">
      ...
      <article><span>{networkText("Aktywne interfejsy")}</span><strong>{state.interfaces.filter((item) => item.state === "up").length}</strong><small>{networkText(networkText("z"))} {state.interfaces.length}</small></article>
      ...
      <article><span>DNS</span><strong>{dnsServers[0] || "—"}</strong><small>{dnsServers.length} {networkText(networkText("serwerów"))}</small></article>
    </section>
    ...
    <section className="network-management-card"><header><div><h3>{networkText("Stan efektywny")}</h3><p>{networkText("Aktualna konfiguracja odczytana z systemu.")}</p></div></header><dl><div><dt>{networkText("Domyślna trasa IPv4")}</dt><dd>{defaults.find((item) => item.family === "ipv4")?.address || "—"}</dd></div><div><dt>{networkText("Domyślna trasa IPv6")}</dt><dd>{defaults.find((item) => item.family === "ipv6")?.address || "—"}</dd></div><div><dt>{networkText("Domeny wyszukiwania")}</dt><dd>{(state.managed.dns?.search_domains || state.dns.resolv_conf.search).join(", ") || "—"}</dd></div><div><dt>systemd-resolved</dt><dd>{state.dns.systemd_resolved.available ? networkText("aktywny") : networkText("niedostępny")}</dd></div><div><dt>{networkText("Łączność podstawowa")}</dt><dd>{state.interfaces.some((item) => item.state === "up" && !item.system) && defaults.length ? networkText("gotowa") : networkText("ograniczona")}</dd></div><div><dt>{networkText("Automatyczny rollback")}</dt><dd>{state.transaction ? `${networkText(networkText("aktywny do"))} ${new Date(state.transaction.deadline * 1000).toLocaleTimeString()}` : networkText("brak oczekującej zmiany")}</dd></div></dl></section>
  ...
  return <Modal title={networkText("Konfiguracja DNS")} onClose={onClose}><form onSubmit={submit}><div className="network-form-grid">
    <div className="network-form-field"><span>{networkText("Serwery DNS")}</span><div className="network-management-stack">
      {servers.map((server, index) => <div className="network-inline-inputs" key={index}>
        ...
        <button type="button" aria-label={`${networkText(networkText("Usuń serwer DNS"))} ${index + 1}`} disabled={servers.length === 1} onClick={() => removeServer(index)}><Trash2 aria-hidden="true" /></button>
      </div>)}
...
function RouteForm({ initial, interfaces, onClose, onSubmit }: { initial?: ManagedNetworkRoute; interfaces: string[]; onClose: () => void; onSubmit: (route: ManagedNetworkRoute) => void }) {
  ...
  return <Modal title={initial ? networkText("Edytuj trasę") : networkText("Nowa trasa statyczna")} onClose={onClose}><form onSubmit={(e) => { e.preventDefault(); onSubmit(value); }}><div className="network-form-grid">
    ...
    <Field label={networkText("Metryka")}><input type="number" min={0} value={value.metric} onChange={(e) => setValue({ ...value, metric: Number(e.target.value) })} /></Field><Field label={networkText("Tabela")}><input type="number" min={1} value={value.table} onChange={(e) => setValue({ ...value, table: Number(e.target.value) })} /></Field><Field label={networkText("Adres źródłowy")}><input value={value.source || ""} onChange={(e) => setValue({ ...value, source: e.target.value || null })} /></Field><Field label="Aktywna"><input type="checkbox" checked={value.enabled} onChange={(e) => setValue({ ...value, enabled: e.target.checked })} /></Field><Field label={networkText("Odtwarzaj po starcie")}><input type="checkbox" checked={value.autostart} onChange={(e) => setValue({ ...value, autostart: e.target.checked })} /></Field>
  </div><footer><button type="button" onClick={onClose}>{networkText("Anuluj")}</button><button className="button-primary">{networkText("Przejdź do planu")}</button></footer></form></Modal>;
}
...
function PlanModal({ plan, busy, error, onClose, onApply }: { plan: NetworkPlan; busy: boolean; error: string; onClose: () => void; onApply: (phrase: string) => void }) {
  ...
  return <Modal title={networkText("Podgląd planu zmian")} onClose={onClose}><div className="network-plan">
    <div className={`network-risk ${plan.high_risk ? "high" : "normal"}`}><ShieldAlert /><div><strong>{plan.high_risk ? networkText("Zmiana wysokiego ryzyka") : networkText("Plan gotowy")}</strong><p>{networkText(networkText("Cel"))}: {plan.target}. {networkText(networkText("Automatyczny rollback po"))} {plan.rollback_seconds} s.</p></div></div>
    ...
    <details open><summary>{networkText(networkText("Polecenia"))} ({plan.commands.length})</summary><pre>{plan.commands.map((command) => command.join(" ")).join("\n") || networkText("Zmiana stanu zarządzanego")}</pre></details>
    ...
    {plan.high_risk && <Field label={`${networkText(networkText("Wpisz dokładnie"))}: ${plan.required_phrase}`}><input autoFocus value={phrase} onChange={(e) => setPhrase(e.target.value)} /></Field>}
...
function TransactionBanner({ transaction, connection, pendingAction, serverOffset, onConfirm, onRollback }: {
  ...
  return <aside className="network-transaction-banner" role="status"><RefreshCw /><div><strong>{title}</strong>
    <p>{active && left > 0 ? `${networkText(networkText("Nowa konfiguracja sieci oczekuje na potwierdzenie. Automatyczne przywrócenie za"))} ${countdown}` : title}</p>
