...
import {
  Accessibility, AlertTriangle, Bell, CheckCircle2, CircleUserRound, FileCog, FolderOpen, Info, KeyRound, Languages,
  ChevronLeft, ChevronRight, Image, MonitorCog, Network, Package, Palette, RefreshCw, ScrollText, Search, Server, Settings, ShieldCheck, SlidersHorizontal, Terminal, Users, X
} from "lucide-react";
...
export type SettingsCategory = "system" | "personalization" | "files" | "transfers" | "notifications" | "accessibility" | "language" | "account" | "identity" | "network" | "networkResources" | "updates" | "policies" | "authentication" | "administration" | "about";
...
export function InterfaceScaleControl({ label, value, t, onChange }: { label: string; value: number; t: Translate; onChange: (value: number) => void }) {
  ...
  return <select className="interface-scale-control" aria-label={label} value={normalizedValue} onChange={(event) => onChange(normalizeInterfaceScale(event.currentTarget.value))}>
    {INTERFACE_SCALE_OPTIONS.map((scale) => <option key={scale} value={scale}>{t(`settings.interfaceScale.${scale}`)}</option>)}
  </select>;
}
...
const categoryIcons: Record<SettingsCategory, ReactNode> = {
  ...
  identity: <Users />, network: <Network />, networkResources: <FolderOpen />, updates: <RefreshCw />, policies: <ScrollText />, authentication: <KeyRound />, administration: <ShieldCheck />, about: <Info />,
};
...
const categorySettings: Record<SettingsCategory, string[]> = {
  ...
  identity: ["usersAndGroups"], network: ["networkMonitor", "dnsDiagnostics", "routingTable"], networkResources: ["networkResources"], updates: ["updates", "updateStatus", "webnasUpdates", "npmPackages", "npmPackagesFrontend", "updateNpmNow"], policies: ["updatePolicies", "automaticUpdateChecks", "updateInterval", "automaticUpdates", "automaticNpmUpdates", "updateConfiguration", "containerDefaultsPolicy", "networkConfirmationTimeout"], authentication: [], administration: ["serviceInformation", "proxmoxSafeMode"], about: ["applicationName", "version", "technologies", "license", "repository"],
};
...
function AdministrationSection({ view, locale, t, toast, onOpenApp }: { view: "administration" | "updates"; locale: "pl-PL" | "en-US"; t: Translate; toast: ToastFn; onOpenApp: (app: AppId) => void }) {
  ...
  const [updatePolicy, setUpdatePolicy] = useState<AutoUpdateSettings | null>(null);
  ...
  useEffect(() => {
    ...
    if (view === "administration") {
      ...
      Promise.allSettled([api.checkUpdates(), api.updateProgress(), api.autoUpdate()]).then((results) => {
        if (!live) return;
  ...
  const refreshUpdates = useCallback(async () => {
    ...
    try {
      ...
      setUpdates(value);
      setUpdateError(value.error || "");
      if (value.checked_at) setUpdatePolicy((current) => current ? { ...current, last_checked: value.checked_at! } : current);
  ...
  const updateCheckInterval = updatePolicy
    ? updatePolicy.check_enabled ? `${updatePolicy.interval_hours} ${t("settings.hoursShort")}` : t("common.disabled")
    : "—";
...
export function SettingsAppView({ settings, initialSection = "system", initialPolicySubject, deepLink, t, toast, onSettingsChange, onOpenApp, onSectionChange, onDeepLinkClose }: {
  ...
  function content() {
    ...
    if (category === "accessibility") return <Card><SettingRow title={t("settings.interfaceScale")} description={t("settings.interfaceScaleDescription")}><InterfaceScaleControl label={t("settings.interfaceScale")} value={settings.interface_scale} t={t} onChange={(value) => void save({ interface_scale: value })} /></SettingRow>{yesNo("reduced_motion", t("settings.reduceMotion"), t("settings.systemPreferencesRespected"))}{yesNo("high_contrast", t("settings.highContrast"), t("settings.systemPreferencesRespected"))}{yesNo("strong_active_borders", t("settings.strongActiveBorders"))}{yesNo("always_show_focus", t("settings.alwaysShowFocus"))}</Card>;
    ...
    if (category === "authentication") return null;
    ...
    return <div className="settings-card-stack"><Card title="WebNAS"><dl className="settings-details"><dt>{t("settings.applicationName")}</dt><dd>WebNAS</dd><dt>{t("settings.version")}</dt><dd>{packageMetadata.version}</dd><dt>{t("settings.frontendEnvironment")}</dt><dd>{window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1" ? "development" : "production"}</dd><dt>{t("settings.backendEnvironment")}</dt><dd>FastAPI / Linux</dd><dt>{t("settings.technologies")}</dt><dd>React · TypeScript · FastAPI · lucide-react</dd><dt>{t("settings.license")}</dt><dd>{t("settings.licenseInfo")}</dd></dl><a className="settings-repository" href="https://github.com/chmajster/Algen-server-web-explorer-panel" target="_blank" rel="noreferrer">{t("settings.repository")}</a></Card></div>;
  }

  return <section className="settings-app">
    <aside className="settings-sidebar"><div className="settings-profile"><Settings /><span><strong>{t("app.settings")}</strong><small>{settings.username}</small></span></div><div className="settings-search"><Search /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("settings.search")} aria-label={t("settings.search")} />{query && <button type="button" aria-label={t("action.clear")} onClick={() => setQuery("")}><X /></button>}</div><nav aria-label={t("settings.categories")}>{categories.map((item) => <button key={item} type="button" className={category === item ? "active" : ""} aria-current={category === item ? "page" : undefined} onClick={() => choose(item)}>{categoryIcons[item]}<span>{categoryLabel(item)}</span></button>)}</nav></aside>
    <main className="settings-main"><header className="settings-header"><div className="settings-title-wrap">{category === "personalization" && personalizationPage === "wallpaper" && !normalizedQuery && <button className="settings-back" type="button" title={t("action.back")} aria-label={t("action.back")} onClick={() => setPersonalizationPage("overview")}><ChevronLeft /></button>}<div><small>{category === "personalization" && personalizationPage === "wallpaper" ? `${t("app.settings")} · ${t("settings.category.personalization")}` : t("app.settings")}</small><h2>{normalizedQuery ? t("settings.searchResults") : category === "personalization" && personalizationPage === "wallpaper" ? t("settings.wallpaper") : categoryLabel(category)}</h2></div></div><div className={`settings-save-state ${saveState}`} role="status" aria-live="polite">{saveState === "saving" ? t("settings.saving") : saveState === "saved" ? t("settings.saved") : saveState === "error" ? `${t("settings.saveError")}: ${saveError}` : ""}</div><Select label={t("settings.categories")} value={category} onChange={(value) => choose(value as SettingsCategory)}>{categories.map((item) => <option key={item} value={item}>{categoryLabel(item)}</option>)}</Select></header><div className={`settings-content ${category === "policies" && !normalizedQuery ? "policy-content" : ""} ${category === "identity" && !normalizedQuery ? "identity-content" : ""} ${category === "personalization" && personalizationPage === "wallpaper" ? "wallpaper-content" : ""}`}>{!normalizedQuery && category === "system" && <HostInformationSection language={settings.language} t={t} />}{normalizedQuery ? <div className="settings-search-results">{searchResults.length ? searchResults.map((result) => <button key={`${result.category}:${result.key}`} type="button" onClick={() => result.category === "personalization" && result.key === "wallpaper" ? openWallpaper() : choose(result.category)}>{categoryIcons[result.category]}<span><strong>{result.label}</strong><small>{categoryLabel(result.category)}</small></span></button>) : <div className="empty-state">{t("settings.noSearchResults")}</div>}</div> : content()}</div></main>
  </section>;
}
