import {
  Accessibility, Bell, CircleUserRound, FileCog, Info, Languages, MonitorCog, Network,
...
export type SettingsCategory = "system" | "personalization" | "files" | "transfers" | "notifications" | "accessibility" | "language" | "account" | "network" | "administration" | "about";
...
const categoryIcons: Record<SettingsCategory, ReactNode> = {
  ...
  network: <Network />, administration: <ShieldCheck />, about: <Info />,
};

const categorySettings: Record<SettingsCategory, string[]> = {
  system: ["startupBehavior", "restoreWindows", "emptyDesktop", "showNotificationCenter", "showClockSeconds", "dateFormat", "welcomeWidget", "resetInterface"],
  ...
  network: ["networkResources"], administration: ["serviceInformation", "updates", "automaticUpdates", "proxmoxSafeMode"], about: ["applicationName", "version", "technologies", "license", "repository"],
};
...
function AdministrationSection({ t, toast, onOpenApp }: { t: Translate; toast: ToastFn; onOpenApp: (app: AppId) => void }) {
  ...
  return <div className="settings-card-stack">
    ...
    <Card title={t("settings.administrationApps")}><div className="settings-app-links">{(["services", "logs", "users", "groups"] as AppId[]).map((app) => <button key={app} type="button" onClick={() => onOpenApp(app)}>{app === "users" ? <Users /> : <SlidersHorizontal />}{t(`app.${app}`)}</button>)}</div></Card>
  </div>;
}
...
export function SettingsAppView({ settings, initialSection = "system", t, toast, onSettingsChange, onOpenApp }: {
  ...
  const categories = useMemo(() => (Object.keys(categoryIcons) as SettingsCategory[]).filter((item) => settings.is_admin || !["network", "administration"].includes(item)), [settings.is_admin]);
  ...
  function content() {
    ...
    if (category === "accessibility") return <Card><SettingRow title={t("settings.interfaceScale")}><Select label={t("settings.interfaceScale")} value={settings.interface_scale} onChange={(value) => void save({ interface_scale: Number(value) as SettingsMe["interface_scale"] })}>{[90, 100, 110, 125].map((value) => <option key={value}>{value}%</option>)}</Select></SettingRow>{yesNo("larger_text", t("settings.largerText"))}{yesNo("reduced_motion", t("settings.reduceMotion"), t("settings.systemPreferencesRespected"))}{yesNo("high_contrast", t("settings.highContrast"), t("settings.systemPreferencesRespected"))}{yesNo("strong_active_borders", t("settings.strongActiveBorders"))}{yesNo("always_show_focus", t("settings.alwaysShowFocus"))}</Card>;
  ...
  return <section className="settings-app">
    ...
    <main className="settings-main"><header className="settings-header"><div><small>{t("app.settings")}</small><h2>{normalizedQuery ? t("settings.searchResults") : t(`settings.category.${category}`)}</h2></div><div className={`settings-save-state ${saveState}`} role="status" aria-live="polite">{saveState === "saving" ? t("settings.saving") : saveState === "saved" ? t("settings.saved") : saveState === "error" ? `${t("settings.saveError")}: ${saveError}` : ""}</div><Select label={t("settings.categories")} value={category} onChange={(value) => choose(value as SettingsCategory)}>{categories.map((item) => <option key={item} value={item}>{t(`settings.category.${item}`)}</option>)}</Select></header><div className="settings-content">{normalizedQuery ? <div className="settings-search-results">{searchResults.length ? searchResults.map((result) => <button key={`${result.category}:${result.key}`} type="button" onClick={() => choose(result.category)}>{categoryIcons[result.category]}<span><strong>{result.label}</strong><small>{t(`settings.category.${result.category}`)}</small></span></button>) : <div className="empty-state">{t("settings.noSearchResults")}</div>}</div> : content()}</div></main>
  </section>;
}
