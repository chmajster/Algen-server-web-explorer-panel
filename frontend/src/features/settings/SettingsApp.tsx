...
const categoryIcons: Record<SettingsCategory, ReactNode> = {
  ...
  identity: <Users />, network: <Network />, administration: <ShieldCheck />, about: <Info />,
};
...
export function SettingsAppView({ settings, initialSection = "system", t, toast, onSettingsChange, onOpenApp }: {
  ...
  const categories = useMemo(() => (Object.keys(categoryIcons) as SettingsCategory[]).filter((item) => settings.is_admin || !["identity", "network", "administration"].includes(item)), [settings.is_admin]);
