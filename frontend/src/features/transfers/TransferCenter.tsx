    ...
    <div className="transfer-list">{visible.length === 0 ? <div className="empty-state"><strong>{t("transfers.empty")}</strong><span>{t("transfers.emptyHint")}</span></div> : visible.map((task) => {
      ...
      return <article key={task.id} className={`transfer-card ${task.status}`}>
        <div className="transfer-summary"><span className="transfer-kind">{t(`transfers.${task.type}`)}</span><div className="transfer-name"><strong>{task.current_file || task.source_paths.map((path) => path.split("/").pop()).join(", ")}</strong><small>{task.source_paths.join(", ")} → {task.destination_path || "—"}</small></div><span className={`status-badge ${task.status}`}>{t(`task.${task.status}`)}</span><div className="transfer-actions">
          {task.status === "running" && <button title={t("transfers.pause")} onClick={() => task.type === "upload" ? uploadControls.pause(task.id) : void run(() => api.pauseTask(task.id))}><Pause /></button>}
          {task.status === "paused" && <button title={t("transfers.resume")} onClick={() => task.type === "upload" ? uploadControls.resume(task.id) : void run(() => api.resumeTask(task.id))}><Play /></button>}
          {["failed", "cancelled"].includes(task.status) && <button title={t("transfers.retry")} onClick={() => task.type === "upload" ? uploadControls.retry(task.id) : void run(() => api.retryTask(task.id))}><RotateCcw /></button>}
          {!done && <button title={t("transfers.cancel")} onClick={() => task.type === "upload" ? uploadControls.cancel(task.id) : void run(() => api.cancelTask(task.id))}><X /></button>}
