...
export const dhcpClient = {
  ...
  createDhcpReservationFromHost: (hostId: string, values: { subnet_id: string; mac_address: string; hostname?: string; create_dns_record?: boolean; dns_provider?: "auto" | "pihole" | "adguard-home" } & Confirmation) => request<{ job: AppJob }>(`/api/modules/dhcp/hosts/${encodeURIComponent(hostId)}/reservation`, { method: "POST", body: JSON.stringify({ ...values, confirm: true, confirmation_text: values.confirmation }) }),
