    ...
    <div className="image-converter-layout">
      <main className="image-converter-main">
        <section className="image-converter-card">
          ...
          </div> : <div className="image-converter-source-content"><div className="image-converter-drop" onDragOver={(event) => event.preventDefault()} onDrop={onDrop}><div className="image-converter-drop-icon"><Upload aria-hidden="true" /></div><strong>{tx.dropTitle}</strong><p>{tx.drop}</p><div className="image-converter-file-actions"><label className="image-converter-button image-converter-button-primary"><FileImage aria-hidden="true" /> {tx.chooseFiles}<input hidden type="file" accept="image/*,.heic,.heif,.avif" multiple onChange={(event) => appendFiles(Array.from(event.target.files || []))} /></label><button className="image-converter-button" onClick={() => directoryInput.current?.click()}><FolderOpen aria-hidden="true" /> {tx.chooseLocalDirectory}</button><input ref={directoryInput} hidden type="file" accept="image/*,.heic,.heif,.avif" multiple onChange={(event) => appendFiles(Array.from(event.target.files || []))} /></div></div>{files.length > 0 && <div className="image-converter-selected"><div className="image-converter-list-title"><strong>{tx.selected}</strong><div><span>{files.length}</span><button onClick={() => setFiles([])}>{tx.clear}</button></div></div>{files.slice(0, 20).map((file, index) => <p key={`${file.name}:${file.size}:${index}`}><span><FileImage aria-hidden="true" /> {file.name}</span><small>{formatBytes(file.size)}</small></p>)}{files.length > 20 && <small className="image-converter-note">+{files.length - 20}</small>}</div>}</div>}
        </section>
