...
describe("responsive taskbar overflow handling", () => {
  it("switches taskbar applications to compact icon-only controls before labels overflow", () => {
    expect(responsive).toContain("@media (max-width: 1400px)");
    expect(responsive).toMatch(/@media \(max-width: 1400px\)[\s\S]*\.desktop \.taskbar-items button span\s*\{\s*display:\s*none;/);
    expect(responsive).toMatch(/@media \(max-width: 1400px\)[\s\S]*\.desktop \.taskbar-items button\s*\{[\s\S]*min-width:\s*2\.75rem;/);
  });
